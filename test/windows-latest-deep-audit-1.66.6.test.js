...
test('1.71.14 version, Windows builds and PWA generation are synchronized', () => {
  assert.equal(pkg.version, '1.71.14');
  assert.equal(lock.version, '1.71.14');
  assert.equal(lock.packages[''].version, '1.71.14');
  assert.match(workflow, /^run-name: v1\.71\.14$/m);
  ...
  for (const rel of ['pwa/theme-init.js','pwa/mobile-intelligence.js','pwa/index.html','pwa/admin-advanced.js','pwa/app.js','pwa/sw.js']) {
    assert.match(read(rel), /1\.71\.14|pwa477/);
  }
