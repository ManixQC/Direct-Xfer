...
test('Windows metadata targets 1.65.0 and current runtime hashes', () => {
  const launcher = read('windows-launcher/Program.cs');
  const host = read('windows-server-host/Program.cs');
  assert.match(launcher, /AppVersion = "1\.65\.0"/);
  assert.match(launcher, /RuntimeAppBuild = "1\.65\.0-launcher61-csharp"/);
  assert.match(host, /HostVersion = "1\.65\.0-serverhost34-csharp"/);
