...
test('1.71.15 SignPath executable configuration uses one release ProductVersion for every Direct-Xfer binary', () => {
...
test('1.71.15 Windows CI builds SignPath inputs with release ProductVersion and component FileVersion checks', () => {
  assert.match(workflow, /DX_VERSION: '1\.71\.15'/);
...
test('Foundation public policy, privacy display and uninstall requirements stay visible', () => {
  ...
  assert.match(installer, /#define AppVersion "1\.71\.15"/);
  assert.match(installer, /#define SourceDir "\.\.\\dist\\Direct-Xfer-1\.71\.15-Windows-CSharp"/);
});
