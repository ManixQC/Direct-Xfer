...
test('1.62.1 release metadata is synchronized across Node, PWA, launcher, host and installer', () => {
  const pkg = JSON.parse(read('package.json'));
  const lock = JSON.parse(read('package-lock.json'));
  assert.equal(pkg.version, '1.62.1');
  assert.equal(lock.version, '1.62.1');
  assert.equal(lock.packages[''].version, '1.62.1');
  assert.match(read('pwa', 'app.js'), /APP_VERSION = '1\.62\.1'/);
  assert.match(read('pwa', 'app.js'), /APP_BUILD = '2026\.08\.15-pwa305'/);
  assert.match(read('pwa', 'sw.js'), /VERSION = '2026\.08\.15-pwa305'/);
  assert.match(read('pwa', 'index.html'), /v1\.62\.1 · pwa305/);
  assert.match(launcher, /AppVersion = "1\.62\.1"/);
  assert.match(launcher, /RuntimeAppBuild = "1\.62\.1-launcher42-csharp"/);
  assert.match(host, /HostVersion = "1\.62\.1-serverhost15-csharp"/);
  assert.match(workflow, /DX_VERSION: '1\.62\.1'/);
  assert.match(workflow, /DX_RUNTIME_BUILD: '1\.62\.1-launcher42-csharp'/);
  assert.match(workflow, /DX_SERVER_HOST_BUILD: '1\.62\.1-serverhost15-csharp'/);
  assert.match(iss, /#define AppVersion "1\.62\.1"/);
});
...
test('PWA and admin notification assets are cache-busted consistently while the favicon remains v269', () => {
  ...
  assert.match(read('pwa', 'index.html'), /app\.js\?v=289/);
  assert.match(read('pwa', 'sw.js'), /app\.css\?v=274/);
  assert.match(read('pwa', 'sw.js'), /app\.js\?v=289/);
