...
test('1.69.6 POST/PATCH connector writes return an authoritative no-store inventory', () => {
  const postStart = adminStorage.indexOf("adminRouter.post('/storage/connectors'");
  const patchStart = adminStorage.indexOf("adminRouter.patch('/storage/connectors/:id'", postStart);
  const deleteStart = adminStorage.indexOf("adminRouter.delete('/storage/connectors/:id'", patchStart);
  assert.ok(postStart >= 0 && patchStart > postStart && deleteStart > patchStart);
  const post = adminStorage.slice(postStart, patchStart);
  const patch = adminStorage.slice(patchStart, deleteStart);
  for (const block of [post, patch]) {
    ...
    assert.match(block, /connector:\s*publicConnector\(/);
    assert.match(block, /connectors/);
  }
});

test('1.69.6 connector reads and generic API GETs bypass caches', () => {
  const summary = adminStorage.slice(
    adminStorage.indexOf("adminRouter.get('/storage/connectors/summary'"),
    adminStorage.indexOf("adminRouter.get('/storage/connectors'", adminStorage.indexOf("adminRouter.get('/storage/connectors/summary'") + 1)
  );
...
test('1.69.6 successful connector save renders the server-confirmed connector before background refresh', () => {
...
test('1.69.6 stale connector refreshes cannot overwrite a newer inventory', () => {
