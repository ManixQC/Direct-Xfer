...
test("rclone Go dependency downloads retry transient network failures without disabling checksum verification", () => {
  ...
  assert.match(dockerfile, /retry_go go get "google\.golang\.org\/grpc@\$\{DX_RCLONE_GRPC_VERSION\}"/);
  assert.match(dockerfile, /test "\$\(go list -m -f '\{\{\.Version\}\}' google\.golang\.org\/grpc\)" = "\$\{DX_RCLONE_GRPC_VERSION\}"/);
