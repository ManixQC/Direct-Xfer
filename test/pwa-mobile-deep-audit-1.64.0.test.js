...
test('1.70.28 release and pwa461 are synchronized',()=>{
  assert.match(read('package.json'),/"version"\s*:\s*"1\.70\.28"/);
  for(const f of ['pwa/app.js','pwa/index.html','pwa/sw.js','pwa/theme-init.js','pwa/admin-advanced.js','pwa/mobile-intelligence.js']) assert.match(read(f),/1\.70\.28|pwa461|v=444/);
...
test('shortcut manifests use the current pwa461 cache buster on app and login paths',()=>{
  assert.match(app,/manifestHref = '\/direct-xfer-pwa\.webmanifest\?v=444'/);
  assert.match(read('pwa/login.html'),/direct-xfer-pwa\.webmanifest\?v=444/);
});
