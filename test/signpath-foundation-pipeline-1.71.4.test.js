...
test('1.71.14 SignPath executable configuration uses one release ProductVersion for every Direct-Xfer binary', () => {
...
test('1.71.14 Windows CI builds SignPath inputs with release ProductVersion and component FileVersion checks', () => {
  assert.match(workflow, /DX_VERSION: '1\.71\.14'/);
...
test('Foundation public policy, privacy display and uninstall requirements stay visible', () => {
  ...
  assert.match(installer, /#define AppVersion "1\.71\.14"/);
  assert.match(installer, /#define SourceDir "\.\.\\dist\\Direct-Xfer-1\.71\.14-Windows-CSharp"/);
});
