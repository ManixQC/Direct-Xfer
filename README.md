...
**Direct-Xfer 1.69.3 hardens Windows release readiness for SignPath Foundation.** The Inno Setup wizard now shows the privacy policy, exposes explicit installation choices for automatic update checks and startup public-IP discovery, and makes Windows autostart a visible task instead of an implicit system change. The selected privacy preferences are applied once to Direct-Xfer settings on first launch after installation or upgrade and remain editable afterwards. Windows publisher/support/update metadata now points to the public GitHub project, release documentation includes the required Code signing policy wording, and SignPath release guidance includes a ready-to-use release-page template. The standard admin interface also exposes public-IP discovery as a privacy setting, and disabled discovery prevents startup calls to public-IP services. Release metadata is synchronized across web, PWA, OAuth broker, Windows launcher/ServerHost, installer and CI.
...
**Direct-Xfer 1.67.44 reorganizes and hardens the standard ‘Edit link’ modal.** The dialog now has a wide responsive layout, grouped sections, a dedicated internal scroll area, and permanently reachable header/save actions on Firefox, 4K displays and short-height windows. It also separates access-rule translation keys from the Admin accounts namespace so geo/IP labels no longer get overwritten by account-management text.

**Direct-Xfer 1.67.42 fixes standard-share DLP warnings on Firefox and other browsers that suppress native confirmation dialogs.** DLP warnings now use a Direct-Xfer in-app modal with explicit **Publish anyway** / **Cancel** actions, show both sensitive findings and incomplete-scan reasons when both apply, and expose the configured per-file DLP scan limit when oversized files were skipped. Cancelling a warning is treated as a normal cancellation instead of the raw `dlp-warning` creation error. The same DLP override helper is used by standard shares, add-to-share, photos and web-storage share/collaboration creation.

**Direct-Xfer 1.67.41 deep-audits the recent Google Drive, rclone, connector and web-storage changes.** Connector capability probes are generation-safe so a slow pre-change probe cannot repopulate stale rclone state after OAuth/configuration changes. GET/HEAD method handling is case-insensitive, connector error codes remain precise end-to-end (including CA trust, disabled Drive API, invalid tokens and config storage), web-storage creation tolerates a bounded background probe while the real browse request remains authoritative, and the PWA now preserves server-confirmed connector inventories during transient refresh failures. Diagnostics redact OAuth secrets more broadly, missing connectors consistently map to not-found, and duplicate web-storage UI work plus generic create errors were cleaned up.

**Direct-Xfer 1.67.40 fixes the web-storage connector preflight and the underlying GET request helper.** GET/HEAD requests can no longer receive a JSON request body when callers pass `null` only as a timeout placeholder; connector summary, connector inventory and remote-browse requests therefore reach the server normally. This fixes the false “Unable to verify connector configuration” error when creating a web-storage link after a connector was already saved.

**Direct-Xfer 1.67.39 fixes stale connector load errors after a valid inventory is already visible.** Once the server has confirmed the connector inventory, a later background refresh failure preserves both the rendered connectors and their non-error status instead of showing the misleading red “Could not load connectors” banner. Exact repeated connector POSTs are also idempotent, so retries after a transient UI refresh cannot create another identical connector record.

**Direct-Xfer 1.67.38 fixes connector inventory disappearing immediately after a successful save.** Connector create/update responses now carry the authoritative persisted inventory, which is rendered before any background probe. API reads bypass caches, refreshes are sequenced so stale responses cannot overwrite newer state, and a transient refresh failure never clears a connector already confirmed by the server.

**Direct-Xfer 1.67.37 fixes persisted Google Drive remote recognition on the Configuration page.** A Google Drive destination that already exists in `rclone.conf` is now treated as already connected instead of as a failed OAuth attempt. Direct-Xfer restores its configured/granted scope metadata without exposing tokens, reloads rclone remotes even when no Direct-Xfer connector record exists yet, and guides the administrator to select the detected destination and add the connector rather than needlessly reconnecting Google. The bounded background rclone probe introduced in 1.67.36 remains in place.

**Direct-Xfer 1.67.35 fixes Google Drive TLS trust for rclone.** Direct-Xfer now builds a private merged CA bundle for rclone from Node's maintained public roots, the operating-system CA bundle and optional administrator-provided PEM certificates. The Docker image explicitly installs and validates `ca-certificates`. An `x509: certificate signed by unknown authority` failure is classified separately and Direct-Xfer never disables certificate verification. Custom PEM roots can be placed in the persistent `rclone/ca` data folder or supplied with `DIRECT_XFER_RCLONE_CA_FILE`.

**Direct-Xfer 1.67.35 makes Google Drive OAuth scope telemetry visible in the connection window.** Before Google opens, Direct-Xfer shows the exact requested permission (for example `drive.file — non-sensitive`). After the broker completes OAuth, the same window shows the exact scope returned by Google through the broker. If the returned grant is broader or otherwise differs from the requested least-privilege scope, Direct-Xfer keeps the actual returned scope visible and displays an explicit mismatch warning.

**Direct-Xfer 1.67.33 fixes first-time rclone configuration storage and makes Google Drive failures diagnosable.** Direct-Xfer now creates and locks down the parent directory for `rclone.conf` before the first remote is written (important when `RCLONE_CONFIG` points to `/data/rclone/rclone.conf` or the Windows data directory), distinguishes the config-write and Google verification phases, classifies common storage/token/API errors, and returns only a sanitized rclone diagnostic to the authenticated admin UI. OAuth tokens, broker handles and client credentials are redacted from diagnostics.

**Direct-Xfer 1.67.31 fixed Google Drive finalization after OAuth.** When Direct-Xfer already holds a Google token (from the public broker or local Web OAuth), it tells `rclone config create` to preserve that token with `config_refresh_token=false` instead of letting rclone start a second OAuth flow. Direct-Xfer also forces password obscuring when writing OAuth client secrets so long base64-like broker secrets cannot be misdetected by rclone.

**Direct-Xfer 1.67.30 uses least-privilege Google Drive authorization by default.** The default access mode is `drive.file`, which lets Direct-Xfer manage files it creates without requesting full access to the user's Drive. Administrators can explicitly choose read-only access to all Drive files (`drive.readonly`) when they need to browse/import existing content, or full access (`drive`) only when existing Drive files must also be modified. The selected scope is preserved end-to-end through the public OAuth broker, local Web OAuth and rclone configuration, and Direct-Xfer rejects a credential that silently comes back broader than the requested limited mode.

The normal user-facing flow is **Connect Google Drive → choose Google account → Allow → done**. For a **public broker reachable from remote browsers**, Direct-Xfer 1.67.29 includes an in-app deployment assistant in **Configuration → Google Drive → OAuth broker → Configure automatically**. The installed application talks directly to the Cloudflare API to create/reuse D1, publish the Worker, configure its secrets, enable its `workers.dev` URL, test it, and save the resulting broker URL. No source checkout, PowerShell, Node/npm, or Wrangler is required on the administrator's workstation.

The assistant asks only for a temporary Cloudflare API token and, on first setup, a Google Web OAuth Client ID/Secret created for the callback displayed by Direct-Xfer. The Cloudflare Account ID is auto-detected when Cloudflare exposes it to the token; otherwise the UI links directly to Cloudflare's Account ID instructions and asks for the 32-character ID once. Since 1.67.29, deployment failures include the exact Cloudflare stage, HTTP status and sanitized API detail instead of the generic deployment-failed message. Finalizing the Google client also waits for Cloudflare to propagate the new GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET bindings instead of failing on the first stale `google:false` health response, and newly-created workers.dev endpoints receive a bounded propagation retry window. The Cloudflare token is kept only in server memory for the deployment session; the Google Client Secret is sent to Cloudflare as a Worker secret and is not saved in Direct-Xfer settings. If you already operate a centrally configured broker, individual Direct-Xfer installations need none of those credentials: simply save its HTTPS URL in the same panel or set `DIRECT_XFER_OAUTH_BROKER_URL` (the environment variable has priority and locks the UI value).

When upgrading an existing installation from 1.67.29 or earlier, run **Configure automatically** once more so the already-published Cloudflare Worker is updated to understand per-session Drive scopes. Direct-Xfer refuses an older Worker that still forces the full Drive scope instead of silently broadening the authorization.

The standalone broker implementation remains under `oauth-broker/` for operators who prefer manual deployment. Broker errors are reported precisely instead of being collapsed into the generic remote-browser/localhost warning. If the broker is unreachable, Direct-Xfer can fall back to an existing local Web OAuth profile; when the browser is on the Direct-Xfer host itself, it can also fall back to rclone's local loopback sign-in. A remote browser still needs either the public broker or a local Web OAuth profile because Google's loopback callback targets the browser machine. See `oauth-broker/README.md`.
