...
function createStorageConnectorConfigRoutes(deps) {
      ...
      if (brokerConfigured) {
        try {
          ...
          const item = {
            ...
            actorUsername:String(req.session && req.session.username || 'admin'), actorIp:String(clientIp(req) || ''),
  ...
  adminRouter.get('/storage/oauth/google/callback', async (req, res) => {
    ...
    try {
      ...
      const tokenResponse = await fetch('https://oauth2.googleapis.com/token', { method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded',Accept:'application/json'}, body });
