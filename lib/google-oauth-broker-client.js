...
class GoogleOAuthBrokerClient {
  ...
  setBaseUrl(value) { this.baseUrl = cleanBrokerUrl(value); return this.baseUrl; }
  forBaseUrl(value) {
    return new GoogleOAuthBrokerClient({ baseUrl:value, fetch:this.fetch, version:this.version, timeoutMs:this.timeoutMs });
  }
  ...
  async createSession(options = {}) {
    ...
    const expectedRedirect = new URL(`${this.baseUrl}/v1/google/callback`).toString();
    const state = String(auth.searchParams.get('state') || '');
    const challenge = String(auth.searchParams.get('code_challenge') || '');
    if (auth.protocol !== 'https:' || auth.hostname !== 'accounts.google.com' || auth.pathname !== '/o/oauth2/v2/auth') {
      throw Object.assign(new Error('oauth-broker-response-invalid'), { code:'oauth-broker-response-invalid' });
    }
    if (auth.searchParams.get('scope') !== requestedScope) {
      // A 1.67.29 (or older) public Worker ignores the requested scope and always
      // returns the restricted full-Drive grant. Make the required broker update
      // explicit instead of silently falling back to a broader authorization.
      throw Object.assign(new Error('oauth-broker-scope-upgrade-required'), { code:'oauth-broker-scope-upgrade-required' });
    }
    if (
      auth.searchParams.get('redirect_uri') !== expectedRedirect ||
      auth.searchParams.get('response_type') !== 'code' ||
