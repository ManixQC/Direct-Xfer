...
- Application/package metadata is synchronized to `1.71.27` across the root package/lockfile, OAuth broker packages, Windows workflow/installer metadata, SBOM, OpenVEX, ASVS release evidence and release-scoped regression fixtures.
- PWA generation advances to `pwa490` with cache-buster `v=471` so clients cannot retain stale release metadata from the previous shell cache.
...
- Security inventory regenerated for 1.71.27 — **957 entries**.
...
- CycloneDX SBOM root component synchronized to 1.71.27.
...
- [x] package and lockfile version synchronized to 1.71.27.
- [x] PWA version/cache generation synchronized to 1.71.27 / pwa490.
...
- [x] SBOM root component synchronized to 1.71.27.
