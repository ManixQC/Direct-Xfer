...
test('CodeQL PWA manifest and image-link preview sinks accept only fixed or pixel-reencoded data', () => {
  const src = read('pwa/app.js');
<<<<<<< HEAD
  assert.match(src, /var manifestHref = '\/direct-xfer-pwa\.webmanifest\?v=447'/);
  assert.match(src, /if \(lang === 'en'\) manifestHref = '\/direct-xfer-pwa-en\.webmanifest\?v=447'/);
=======
  assert.match(src, /var manifestHref = '\/direct-xfer-pwa\.webmanifest\?v=448'/);
  assert.match(src, /if \(lang === 'en'\) manifestHref = '\/direct-xfer-pwa-en\.webmanifest\?v=448'/);
>>>>>>> eb50626 (v1.71.4)
