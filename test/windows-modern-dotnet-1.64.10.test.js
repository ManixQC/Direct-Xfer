...
test('SDK-generated assembly metadata replaces manual assembly attributes', () => {
  ...
  for (const project of [launcherProject, hostProject]) {
    ...
    assert.match(project, /<AssemblyVersion>1\.68\.1\.0<\/AssemblyVersion>/);
    assert.match(project, /<FileVersion>1\.68\.1\.0<\/FileVersion>/);
    assert.match(project, /<InformationalVersion>1\.68\.1-(?:launcher125|serverhost98)-csharp<\/InformationalVersion>/);
  }
