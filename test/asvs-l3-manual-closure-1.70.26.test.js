...
test('signed deployment evidence validates all formerly external ASVS requirements', (t) => {
  ...
  const result=verifyEvidenceBundle(ev.bundle,{ appVersion:'1.71.13', publicUrl:'https://direct-xfer.example', publicKey:ev.keyPair.publicKey });
...
test('deployment evidence is release and public-origin bound', (t) => {
  ...
  assert.equal(verifyEvidenceBundle(ev.bundle,{appVersion:'1.71.13',publicUrl:'https://other.example',publicKey:ev.keyPair.publicKey}).ok,false);
});
...
test('deployment evidence rejects forged observations and stale expiry', (t) => {
  ...
  assert.equal(verifyEvidenceBundle(forged,{appVersion:'1.71.13',publicUrl:'https://direct-xfer.example',publicKey:ev.keyPair.publicKey}).ok,false);
  assert.equal(verifyEvidenceBundle(ev.bundle,{appVersion:'1.71.13',publicUrl:'https://direct-xfer.example',publicKey:ev.keyPair.publicKey,now:ev.bundle.expiresAt+1}).ok,false);
});
