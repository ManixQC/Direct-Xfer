...
test('1.59.5 Windows server metadata is synchronized', () => {
  assert.match(host, /AppVersion = "1\.59\.5"/);
  ...
  assert.match(launcher, /ServerHostBuild = "1\.59\.5-serverhost4-csharp"/);
...
test('all ServerHost critical runtime hashes match the exact normalized 1.59.5 runtime files', () => {
