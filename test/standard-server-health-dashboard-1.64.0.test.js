...
test('standard dashboards expose a dedicated server health tab and assets',()=>{
  assert.match(html,/id="dashboard-health-tab"/);
  assert.match(html,/id="dashboard-health-view"/);
  ...
  assert.match(app,/\['transfers', 'images', 'health'\]/);
...
test('health UI hides itself from non owner-admin roles',()=>{
  ...
  assert.match(app,/tab === 'health'.*role.*owner.*admin/s);
});
...
test('real server protects and serves the complete health dashboard',{timeout:30000},async()=>{
  ...
  try{
    ...
    for(const asset of ['/server-health-dashboard.js?v=2','/server-health-dashboard.css?v=2']){const r=await fetch(base+asset);assert.equal(r.status,200);}
