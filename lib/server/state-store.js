...
// Keep password-derived DATA_KEY behavior stable across Node upgrades. These are
// the same approved scrypt parameters used by the authentication boundary.
...
function createStateStore(options = {}) {
  ...
  if (asvsL3Mode && !externalCrypto) {
    const error = new Error('asvs-l3-external-crypto-provider-required');
    error.code = 'ASVS_L3_CRYPTO_PROVIDER_REQUIRED';
    throw error;
  }
  if (asvsL3Mode && secret) {
    const error = new Error('asvs-l3-data-key-must-not-enter-app-process');
    error.code = 'ASVS_L3_LOCAL_KEY_FORBIDDEN';
    throw error;
  }
  ...
  function deriveDataKey(salt) {
    ...
    const key = crypto.scryptSync(secret, salt, 32, SCRYPT_OPTIONS);
  ...
  function encryptStore(json) {
    if (externalCrypto) {
      const encrypted = externalCrypto.encrypt(String(json), 'direct-xfer-state-v2');
      return JSON.stringify({ dxenc:2, provider:'external', keyId:encrypted.keyId, data:encrypted.ciphertext });
    }
  ...
  function decryptStore(envelope) {
    if (envelope && envelope.dxenc === 2) {
      if (!externalCrypto || envelope.provider !== 'external' || typeof envelope.data !== 'string') {
        const error = new Error('external-crypto-provider-required');
        error.code = 'ASVS_L3_CRYPTO_PROVIDER_REQUIRED';
        throw error;
      }
      return externalCrypto.decrypt(envelope.data, 'direct-xfer-state-v2', envelope.keyId || '');
    }
    if (externalCrypto) {
      const error = new Error('asvs-l3-legacy-store-migration-required');
      error.code = 'ASVS_L3_LEGACY_STORE_MIGRATION_REQUIRED';
      throw error;
    }
  ...
  function serializeState() {
    ...
    return (externalCrypto || secret) ? encryptStore(json) : json;
  }

  function deserializeStore(raw) {
    const obj = JSON.parse(raw);
    if (obj && obj.dxenc) {
      if (!secret && !externalCrypto) {
      ...
      try {
        ...
        if (cause && ['ASVS_L3_LEGACY_STORE_MIGRATION_REQUIRED','ASVS_L3_CRYPTO_PROVIDER_REQUIRED'].includes(cause.code)) throw cause;
        if (cause && String(cause.code || '').startsWith('asvs-crypto-')) throw cause;
    ...
    if (externalCrypto) {
      const error = new Error('asvs-l3-plaintext-store-forbidden');
      error.code = 'ASVS_L3_PLAINTEXT_STORE_FORBIDDEN';
      throw error;
    }
    return obj;
  }
