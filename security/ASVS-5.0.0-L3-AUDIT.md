...
- Application/package metadata is synchronized to `1.71.28` across the root package/lockfile, OAuth broker packages, Windows workflow/installer metadata, SBOM, OpenVEX, ASVS release evidence and release-scoped regression fixtures.
- PWA generation advances to `pwa491` with cache-buster `v=472` so clients cannot retain stale release metadata from the previous shell cache.
...
- Security inventory regenerated for 1.71.28 — **957 entries**.
...
- CycloneDX SBOM root component synchronized to 1.71.28.
...
- [x] package and lockfile version synchronized to 1.71.28.
- [x] PWA version/cache generation synchronized to 1.71.28 / pwa491.
...
- [x] SBOM root component synchronized to 1.71.28.
