...
test('1.69.4 release and pwa411 are synchronized',()=>{
  assert.match(read('package.json'),/"version"\s*:\s*"1\.69\.4"/);
  for(const f of ['pwa/app.js','pwa/index.html','pwa/sw.js','pwa/theme-init.js','pwa/admin-advanced.js','pwa/mobile-intelligence.js']) assert.match(read(f),/1\.69\.4|pwa411|v=401/);
...
test('shortcut manifests use the current pwa411 cache buster on app and login paths',()=>{
  assert.match(app,/direct-xfer-pwa\.webmanifest'[\s\S]*?\+ '\?v=401'/);
  assert.match(read('pwa/login.html'),/direct-xfer-pwa\.webmanifest\?v=401/);
});
