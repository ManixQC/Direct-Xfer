...
test('1.71.12 release and pwa475 are synchronized',()=>{
  assert.match(read('package.json'),/"version"\s*:\s*"1\.71\.12"/);
  for(const f of ['pwa/app.js','pwa/index.html','pwa/sw.js','pwa/theme-init.js','pwa/admin-advanced.js','pwa/mobile-intelligence.js']) assert.match(read(f),/1\.71\.12|pwa475|v=456/);
...
test('shortcut manifests use the current pwa475 cache buster on app and login paths',()=>{
  assert.match(app,/manifestHref = '\/direct-xfer-pwa\.webmanifest\?v=456'/);
  assert.match(read('pwa/login.html'),/direct-xfer-pwa\.webmanifest\?v=456/);
});
