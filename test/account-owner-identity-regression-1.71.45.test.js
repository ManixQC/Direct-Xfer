...
test('HTTP/PWA lifecycle resolves the owner login through a stable function instead of the dynamic adminUsername getter', () => {
  const fs = require('node:fs');
  const source = fs.readFileSync(path.join(__dirname, '..', 'lib', 'server', 'http-pwa-lifecycle-application.js'), 'utf8');
  assert.match(source, /const ownerLoginUsername = stableFunction\(account, 'ownerLoginUsername', 'account'\);/);
  assert.match(source, /ADMIN_USERNAME:ownerLoginUsername\(\),/);
  assert.doesNotMatch(source, /stableValue\(account, 'adminUsername', 'account'\)/);
});

test('dynamic adminUsername remains an accessor while ownerLoginUsername is a stable own function', () => {
  const state = {
    meta:{ accounts:[{
      id:'owner-id', username:'persisted-owner', ah:'hash:existing', role:'owner',
      totp:null, pwChanged:true, createdAt:1, createdBy:'system', lastLoginAt:0,
    }] },
    settings:{ pwChanged:true },
  };
  const service = createService(state);
  const usernameDescriptor = Object.getOwnPropertyDescriptor(service, 'adminUsername');
  const loginDescriptor = Object.getOwnPropertyDescriptor(service, 'ownerLoginUsername');

  assert.equal(typeof usernameDescriptor.get, 'function');
  assert.equal(Object.hasOwn(usernameDescriptor, 'value'), false);
  assert.equal(typeof loginDescriptor.value, 'function');

  service.initialize();
  assert.equal(loginDescriptor.value.call(service), 'persisted-owner');
  assert.equal(service.adminUsername, 'persisted-owner');
});
