# Direct-Xfer 1.71.14 — ASVS L3 release evidence
...
|---|---|---|
| Release-targeted regression tests | PASS | 79 passed, 0 failed, 0 skipped, including dependency floors, npm-audit CI, version/PWA synchronization, Windows metadata and SignPath release invariants |
...
| Security inventory | PASS | Regenerated for 1.71.14; 967 inventory entries |
...
| CycloneDX SBOM | PASS | Root component synchronized to Direct-Xfer 1.71.14 |
...
The 1.71.14 packaging pass executed 79 release-targeted tests with zero failures or skips, plus both repository-verifiable ASVS audits. A full `npm test` run must still be performed by CI after `npm ci`, because this source ZIP deliberately excludes `node_modules` and the local packaging sandbox could not complete registry access.

## Commands/gates used

```text
node --test test/dependency-security-floors-1.71.8.test.js test/npm-audit-ci-1.71.8.test.js test/windows-latest-deep-audit-1.66.6.test.js test/pwa-mobile-deep-audit-1.64.0.test.js test/signpath-foundation-pipeline-1.71.4.test.js test/project-reconstruction-1.64.0.test.js test/windows-recent-deep-audit-1.66.4.test.js test/windows-modern-dotnet-1.64.10.test.js test/release-maintenance-1.71.14.test.js test/trivy-container-hardening-1.71.14.test.js test/windows-passkey-loopback-1.71.6.test.js
