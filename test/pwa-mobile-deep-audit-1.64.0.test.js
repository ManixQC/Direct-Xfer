...
test('1.70.24 release and pwa457 are synchronized',()=>{
  assert.match(read('package.json'),/"version"\s*:\s*"1\.70\.24"/);
  for(const f of ['pwa/app.js','pwa/index.html','pwa/sw.js','pwa/theme-init.js','pwa/admin-advanced.js','pwa/mobile-intelligence.js']) assert.match(read(f),/1\.70\.24|pwa457|v=442/);
...
test('shortcut manifests use the current pwa457 cache buster on app and login paths',()=>{
  assert.match(app,/manifestHref = '\/direct-xfer-pwa\.webmanifest\?v=442'/);
  assert.match(read('pwa/login.html'),/direct-xfer-pwa\.webmanifest\?v=442/);
});
