...
test('device-only status lists all devices paired to its account', async()=>{
  ...
  for (const current of [phone, tablet]) {
    ...
    const statusUrl=`${base}/app/device/status?version=${encodeURIComponent('1.62.2')}&build=${encodeURIComponent('2026.08.16-pwa306')}`;
    ...
    assert.equal(currentDevice.appVersion,'1.62.2');
    assert.equal(currentDevice.appBuild,'2026.08.16-pwa306');
  }
});
