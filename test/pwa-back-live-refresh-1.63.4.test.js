...
test('1.63.4 PWA live transfers refresh every five seconds', () => {
...
test('1.63.4 live transfer heading is 50 percent larger', () => {
  const css = read('pwa','app.css');
  assert.match(css, /\.pwa-live-transfers-head h2,\.pwa-live-transfers-head h3\{[^}]*font-size:1\.5rem/);
});

test('1.63.4 installed PWA double-back exit uses CloseWatcher rather than history stacking', () => {
  const app = read('pwa','app.js');
...
test('1.63.4 PWA assets are cache-busted', () => {
