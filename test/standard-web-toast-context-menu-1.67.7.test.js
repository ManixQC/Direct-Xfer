...
test('1.67.26 web-link creation diagnostics stay visible for ten seconds', () => {
  ...
  assert.match(app, /if\(!available && !pending\) \{ webStorageToast\(t\('webStorage\.rcloneMissing'\),'err'\); return; \}/);
  assert.match(app, /function toast\(msg, kind, durationMs\)/);
  assert.match(app, /Math\.min\(requested, 60000\) : 2600/);
});

test('1.67.26 share context menu has an opaque theme-aware surface', () => {
