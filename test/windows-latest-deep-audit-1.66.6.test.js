...
test('1.69.3 version, Windows builds and PWA generation are synchronized', () => {
  assert.equal(pkg.version, '1.69.3');
  assert.equal(lock.version, '1.69.3');
  assert.equal(lock.packages[''].version, '1.69.3');
  assert.match(workflow, /^run-name: v1\.69\.3$/m);
  assert.match(workflow, /DX_RUNTIME_BUILD: '1\.69\.3-launcher126-csharp'/);
  assert.match(workflow, /DX_SERVER_HOST_BUILD: '1\.69\.3-serverhost99-csharp'/);
  assert.match(launcher, /AppVersion = "1\.69\.3"/);
  assert.match(host, /AppVersion = "1\.69\.3"/);
  assert.match(read('windows-launcher/app.manifest'), /assemblyIdentity version="1\.69\.3\.0"/);
  assert.match(read('windows-server-host/app.manifest'), /assemblyIdentity version="1\.69\.3\.0"/);
  for (const rel of ['pwa/theme-init.js','pwa/mobile-intelligence.js','pwa/index.html','pwa/admin-advanced.js','pwa/app.js','pwa/sw.js']) {
    assert.match(read(rel), /1\.69\.3|pwa410/);
  }
