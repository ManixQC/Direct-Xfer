...
test('1.59.4 Windows server metadata is synchronized', () => {
  ...
  assert.match(launcher, /ServerHostBuild = "1\.59\.4-serverhost3-csharp"/);
  assert.match(workflow, /DX_VERSION: '1\.59\.4'/);
  assert.match(workflow, /DX_SERVER_HOST_BUILD: '1\.59\.4-serverhost3-csharp'/);
});
...
test('all ServerHost critical runtime hashes match the exact normalized 1.59.4 runtime files', () => {
