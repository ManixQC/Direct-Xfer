...
test('1.69.2 preserves precise recent connector errors and HTTP semantics', () => {
...
test('1.69.2 rclone diagnostics redact plain and JSON OAuth secrets', () => {
...
test('1.69.2 connector probe cache rejects pre-mutation stale snapshots', () => {
...
test('1.69.2 web-storage modal accepts a confirmed connector while the bounded rclone probe is pending', () => {
...
test('1.69.2 PWA preserves authoritative connector inventory and clears stale error styling', () => {
...
test('1.69.2 public web-storage missing connector consistently maps to not-found', () => {
  assert.match(server, /code === 'remote-not-found' \|\| code === 'connector-not-found'/);
  assert.match(server, /code==='remote-not-found'\|\|code==='connector-not-found'/);
});

test('1.69.2 server remains under the enforced modularization ceiling', () => {
