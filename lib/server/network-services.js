...
async function fetchText(url, opts = {}, timeoutMs = 6000) {
  ...
  try {
    ...
    const res = await fetch(url, { ...opts, signal: ctrl.signal, redirect: ASVS_L3_MODE === true ? 'error' : 'follow' });
...
async function getPublicIP(force = false) {
  if (publicIpDiscoveryEnabled && !publicIpDiscoveryEnabled()) return null;
  const now = Date.now();
...
// Fast country lookup from the in-memory cache. This never performs network I/O
// and is safe to use on hot request paths before falling back to geolocate().
function geoSync(ip) {
  ...
  return cached && Date.now() - cached.at < GEO_TTL ? cached : null;
}
