...
test('Windows metadata targets 1.66.0 and current runtime hashes', () => {
  const launcher = read('windows-launcher/Program.cs');
  const host = read('windows-server-host/Program.cs');
  assert.match(launcher, /AppVersion = "1\.66\.0"/);
  assert.match(launcher, /RuntimeAppBuild = "1\.66\.0-launcher71-csharp"/);
  assert.match(host, /HostVersion = "1\.66\.0-serverhost44-csharp"/);
...
test('Windows GitHub Actions run name follows Direct-Xfer 1.66.0', () => {
  const workflow = read('.github/workflows/build-windows-csharp.yml');
  assert.match(workflow, /^run-name: v1\.66\.0$/m);
});
