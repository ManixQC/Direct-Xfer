...
const { strictFileContentReason } = require('../file-type-policy');
...
function createUploadReceptionService(deps = {}) {
  const {
    APP_NAME,
    ASVS_L3_MODE = false,
    ASVS_L3_MAX_FILES_PER_SENDER = 1000,
    ASVS_L3_MAX_BYTES_PER_SENDER = 20 * 1024 ** 3,
    CLAMAV_HOST,
    CLAMAV_PORT,
    CLAMAV_SOCKET = '',
    CLAMAV_TLS = false,
    CLAMAV_TLS_SERVERNAME = '',
    CLAMAV_TLS_CA_FILE = '',
    DATA_DIR,
    FULL_IMAGES_DIR,
    INBOX_DIR,
  ...
  async function inboxContentReason(s, absPath, filename = null) {
    // L3 treats every public reception as an untrusted file-upload boundary:
    // extension and content must agree even when the link owner did not manually
    // enable the legacy `blockExecutables` toggle.
    if (ASVS_L3_MODE) {
      try { return await strictFileContentReason(absPath, filename || path.basename(absPath)); }
      catch (_) { return 'file-type-unverified'; }
    }
  ...
  function senderCaps(s) {
    const configuredFiles = Math.max(0, Number(s && s.maxFilesPerSender) || 0);
    const configuredBytes = Math.max(0, Number(s && s.maxBytesPerSender) || 0);
    return {
      files:configuredFiles || (ASVS_L3_MODE ? Math.max(1, Number(ASVS_L3_MAX_FILES_PER_SENDER) || 1000) : 0),
      bytes:configuredBytes || (ASVS_L3_MODE ? Math.max(1024 * 1024, Number(ASVS_L3_MAX_BYTES_PER_SENDER) || 20 * 1024 ** 3) : 0),
    };
  }
  function perSenderRejectReason(s, req, senderName, sizeHint, opts = {}) {
    const caps = senderCaps(s);
    const fCap = caps.files;
    const bCap = caps.bytes;
  ...
  function scanFile(absPath) {
    return new Promise((resolve) => {
      ...
      try {
        if (CLAMAV_SOCKET) {
          socket = net.createConnection({ path:CLAMAV_SOCKET });
        } else if (CLAMAV_TLS) {
          let ca;
          if (CLAMAV_TLS_CA_FILE) ca = fs.readFileSync(CLAMAV_TLS_CA_FILE);
          const tlsOptions = { host:CLAMAV_HOST, port:CLAMAV_PORT, rejectUnauthorized:true };
          if (CLAMAV_TLS_SERVERNAME) tlsOptions.servername = CLAMAV_TLS_SERVERNAME;
          if (ca) tlsOptions.ca = ca;
          socket = tls.connect(tlsOptions);
        } else {
          socket = net.createConnection({ host:CLAMAV_HOST, port:CLAMAV_PORT });
        }
      } catch (error) {
        return resolve({ error:String(error && error.message || 'clamav-connect-failed') });
      }
      let reply = '';
      const handle = () => {
  ...
  async function scanGate(part, name, s, req) {
    ...
    if (r.error) {
      const message = String(r.error || 'scanner-unavailable').slice(0,120);
      emitLiveActivity('antivirus', { shareId:s && s.id, name, status:'error', detail:message, ip:pubIp(clientIp(req)) });
      if (ASVS_L3_MODE) {
        console.error('[clamav] L3 scan error (upload blocked):', message);
        logAudit('upload-scan-blocked', { username:'system', ip:clientIp(req), detail:`${(s && (s.name || s.id)) || 'share'}: ${name} [${message}]` });
        try { await fs.promises.mkdir(QUARANTINE_DIR, { recursive:true }); } catch (_) {}
        const dest = path.join(QUARANTINE_DIR, crypto.randomBytes(8).toString('hex') + '-unscanned-' + safeUploadName(name));
        try { await fs.promises.rename(part, dest); } catch (_) { try { await fs.promises.unlink(part); } catch (_) {} }
        return false;
      }
      console.error('[clamav] scan error (delivering unscanned):', message);
