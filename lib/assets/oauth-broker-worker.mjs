...
function html(title, message, status = 200, close = false, extraHeaders = {}) {
  ...
  return new Response(body, {
    ...
    headers: {
      ...
      'x-content-type-options': 'nosniff',
      'referrer-policy': 'no-referrer',
      'content-security-policy': `default-src 'none'; script-src 'nonce-${nonce}'; style-src 'nonce-${nonce}'; style-src-attr 'none'; script-src-attr 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'; object-src 'none'`,
      ...extraHeaders,
    },
  });
}
