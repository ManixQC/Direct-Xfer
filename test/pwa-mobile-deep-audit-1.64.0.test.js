...
test('1.68.1 release and pwa406 are synchronized',()=>{
  assert.match(read('package.json'),/"version"\s*:\s*"1\.68\.1"/);
  for(const f of ['pwa/app.js','pwa/index.html','pwa/sw.js','pwa/theme-init.js','pwa/admin-advanced.js','pwa/mobile-intelligence.js']) assert.match(read(f),/1\.68\.1|pwa406|v=396/);
...
test('shortcut manifests use the current pwa406 cache buster on app and login paths',()=>{
  assert.match(app,/direct-xfer-pwa\.webmanifest'[\s\S]*?\+ '\?v=396'/);
  assert.match(read('pwa/login.html'),/direct-xfer-pwa\.webmanifest\?v=396/);
});
