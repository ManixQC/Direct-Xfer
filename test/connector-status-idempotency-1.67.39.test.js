...
test('1.69.6 confirmed connector inventory is not replaced by a stale red load error', () => {
...
test('1.69.6 successful POST or PATCH confirms the inventory before background refresh', () => {
...
test('1.69.6 exact connector POST retries are idempotent', () => {
  const start = adminStorage.indexOf("adminRouter.post('/storage/connectors'");
  const end = adminStorage.indexOf("adminRouter.patch('/storage/connectors/:id'", start);
  ...
  assert.match(block, /duplicate:\s*true/);
