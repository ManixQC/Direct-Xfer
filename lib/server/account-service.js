...
function createAccountService(options = {}) {
  ...
  const asvsL3Mode = options.ASVS_L3_MODE === true;
  const predictableAdminNames = new Set(['admin','administrator','root','owner','direct-xfer','directxfer']);
  ...
  // The persisted state is loaded after account-service is constructed. Do not
  // attempt to infer the existing owner here: getState() can still point at the
  // empty bootstrap cell, which previously caused legacy upgrades to preselect a
  // random `owner-...` identifier before their real credentials were loaded.
  // Compatibility mode keeps the historical `admin` identity; ASVS L3 randomizes
  // only a genuinely fresh owner bootstrap.
  let adminUsername = (
    configuredAdminUsername
    || (asvsL3Mode ? `owner-${crypto.randomBytes(6).toString('hex')}` : 'admin')
  ).slice(0, 40);

  function validateAdminUsername(username, enforceL3Policy = true) {
    const value = String(username || '').trim().slice(0, 40);
    if (!value || /[\u0000-\u001f\u007f]/.test(value)) {
      throw new TypeError('account-service requires a valid ADMIN_USERNAME');
    }
    if (enforceL3Policy && asvsL3Mode && predictableAdminNames.has(value.toLowerCase())) {
      const error = new Error('ASVS L3 refuses a predictable owner username; rename the owner in compatibility mode before enabling ASVS_L3_MODE');
      error.code = 'asvs-l3-predictable-admin-username';
      throw error;
    }
    return value;
  }

  adminUsername = validateAdminUsername(adminUsername, true);

  function warnLegacyPredictableOwner(username) {
    if (!asvsL3Mode || !predictableAdminNames.has(String(username || '').trim().toLowerCase())) return;
    warn('predictableLegacyOwner');
  }

  function usePersistedOwnerUsername(owner) {
    if (configuredAdminUsername || !owner || typeof owner.username !== 'string') return;
    // Persisted account identity is authentication state, not disposable bootstrap
    // metadata. Never turn an in-place upgrade into a service outage merely because
    // an older owner name is predictable. Fresh L3 installs still receive a random
    // owner name, while legacy deployments are allowed to start and can rename the
    // account deliberately after authentication.
    adminUsername = validateAdminUsername(owner.username, false);
    warnLegacyPredictableOwner(adminUsername);
  }

  function legacyOwnerUsername() {
    // The pre-multi-account model had one implicit administrator named `admin`
    // unless ADMIN_USERNAME explicitly overrode it. Preserve that recoverable
    // identity during migration even in L3; rejecting it here would crash startup
    // before the administrator has any opportunity to rename the account.
    const username = validateAdminUsername(configuredAdminUsername || 'admin', !!configuredAdminUsername);
    warnLegacyPredictableOwner(username);
    return username;
  }
  ...
  const ACCOUNT_WARNINGS = Object.freeze({
    predictableLegacyOwner:'[accounts] ASVS L3 preserved a predictable legacy owner username for upgrade compatibility; rename it from the admin interface when practical.',
    legacyPasswordRead:'[accounts] could not read legacy admin-password.txt.',
    legacyPasswordRemove:'[accounts] could not remove legacy admin-password.txt.',
    bootstrapPersist:'[accounts] could not persist account bootstrap.',
    generic:'[accounts] warning',
  });

  function warn(code) {
    // Log only catalogued fixed text. Never forward the caller value itself: future
    // call sites may be reached by process.env, paths, usernames or exception data.
    // Mapping through a closed catalogue keeps this sink non-sensitive by construction.
    const message = Object.hasOwn(ACCOUNT_WARNINGS, code) ? ACCOUNT_WARNINGS[code] : ACCOUNT_WARNINGS.generic;
    try { logger.warn(message); } catch (_) {}
  }
  ...
  function makeOwner(meta, settings, ownerHash, migrated, username = adminUsername) {
    return {
      id:newAccountId(),
      username:validateAdminUsername(username, false),
      ah:ownerHash,
      role:'owner',
  ...
  function prepareRestoredState(candidate) {
    ...
    if (!inspection.owner) {
      ...
      const restoredUsername = migratedHash ? legacyOwnerUsername() : adminUsername;
      accounts.push(makeOwner(meta, candidate.settings, ownerHash, !!migratedHash, restoredUsername));
  ...
  function initialize() {
    ...
    if (existingInspection.owner) {
      usePersistedOwnerUsername(existingInspection.owner);
    ...
    if (migratedHash && !configuredAdminUsername) adminUsername = legacyOwnerUsername();
    ...
    if (!ownerHash) {
      if (environmentPasswordManaged) {
        ...
        initialPasswordPlaintext = crypto.randomBytes(asvsL3Mode ? 18 : 12).toString('base64url');
  ...
  return {
    get adminUsername() { return adminUsername; },
    dummyPasswordRecord,
    initialize,
    bootstrapStatus,
    normalizeUsername,
