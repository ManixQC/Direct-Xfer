...
test('PWA cache build is synchronized to pwa346', () => {
  assert.match(theme, /2026\.08\.18-pwa346/);
  assert.match(theme, /admin-advanced\.js\?v=346/);
  assert.match(sw, /2026\.08\.18-pwa346/);
  assert.match(sw, /admin-advanced\.js\?v=346/);
  assert.match(read('pwa/app.js'), /2026\.08\.18-pwa346/);
  assert.match(read('pwa/login.js'), /v=346/);
  assert.match(read('pwa/index.html'), /pwa346/);
...
test('advanced-admin script is a public static PWA asset so first service-worker install can cache it', () => {
  ...
  assert.match(sw, /'\/app\/admin-advanced\.js\?v=346'/);
});
...
test('real server keeps new endpoints protected, serves PWA asset pre-login, and panic-pauses a scheduled share', { timeout:30000 }, async () => {
  ...
  try {
    ...
    const publicAsset = await fetch(base + '/app/admin-advanced.js?v=346');
    assert.equal(publicAsset.status, 200);
    assert.match(await publicAsset.text(), /pwa346/);
