...
// Health needs to describe the URL clients actually use, not just the socket
// protocol between Direct-Xfer and a reverse proxy. Express already folds a
// trusted X-Forwarded-Proto into req.protocol, while a configured public URL is
// authoritative even when the proxy terminates TLS before the Node process.
function resolveHealthPublicScheme(req, deep) {
  const requestScheme = normalizedHttpScheme(req && req.protocol);
  if (requestScheme === 'https') return { scheme:'https', source:'request' };
  const publicUrl = deep && deep.config && deep.config.publicUrl;
  const configuredScheme = configuredPublicScheme(publicUrl);
  if (configuredScheme) return { scheme:configuredScheme, source:'public-url' };
  if (requestScheme) return { scheme:requestScheme, source:'request' };
  const originScheme = normalizedHttpScheme(deep && deep.config && deep.config.scheme) || 'http';
  return { scheme:originScheme, source:'origin' };
}

function projectHealthForRequest(req, deep) {
  if (!deep || typeof deep !== 'object') return deep;
  const config = deep.config && typeof deep.config === 'object' ? deep.config : {};
  const originScheme = normalizedHttpScheme(config.originScheme || config.scheme) || 'http';
  const resolved = resolveHealthPublicScheme(req, deep);
  return {
    ...deep,
    config:{ ...config, originScheme, scheme:resolved.scheme, schemeSource:resolved.source },
  };
}
...
function attachAdminDashboardRoutes(deps = {}) {
  ...
  adminRouter.get('/server-health-dashboard', requireFullAdmin, async (req, res) => {
    ...
    let deep = null; try { deep = projectHealthForRequest(req, await serverHealthDeepSnapshot()); } catch (_) { deep = { error:'unavailable' }; }
...
module.exports = { attachAdminDashboardRoutes, resolveHealthPublicScheme, projectHealthForRequest };
