...
test('device-only status lists all devices paired to its account', async()=>{
  ...
  for (const current of [phone, tablet]) {
    ...
    const statusUrl=`${base}/app/device/status?version=${encodeURIComponent('1.59.0')}&build=${encodeURIComponent('2026.08.14-pwa279')}`;
    ...
    assert.equal(currentDevice.appVersion,'1.59.0');
    assert.equal(currentDevice.appBuild,'2026.08.14-pwa279');
  }
});
