...
function safeLocalTarget(root, relative) {
  ...
  if (rel === null) throw Object.assign(new Error('invalid-local-path'), { code:'EINVAL' });
...
class StorageConnectorService {
    ...
    return new Promise((resolve, reject) => {
      ...
      child.once('close', (code, signal) => {
        ...
        error.code = options.signal && options.signal.aborted ? 'connector-cancelled'
          : timedOut ? 'connector-timeout' : signal ? 'connector-terminated' : 'connector-failed';
        error.exitCode = code;
        reject(error);
      });
    });
  ...
  async list(connector, relative) {
    const result = await this.run([
      'lsjson', remoteSpec(connector, relative), '--max-depth', '1', '--no-mimetype', '--no-modtime',
    ...
    return rows.slice(0, 5000).map((row) => {
      ...
      const safePath = cleanRelativePath(row.Path || row.Name || '', false);
      const safeName = String(row.Name || path.posix.basename(String(row.Path || ''))).trim().slice(0, 255);
      if (safePath === null || !safeName || /[\0-\x1f\x7f]/.test(safeName)) return null;
      return {
        name:safeName, path:safePath, isDir:!!row.IsDir,
  ...
  async cleanupStaleImports() {
    let staging;
    try { staging = await this.ensureStagingRoot(); }
    catch (error) { if (error && error.code === 'ENOENT') return 0; throw error; }
    let entries = [];
    try { entries = await fs.promises.readdir(staging, { withFileTypes:true }); } catch (_) { return 0; }
    let removed = 0;
    for (const entry of entries) {
      if (!entry || !/^job-[A-Za-z0-9-]{12,100}$/.test(entry.name)) continue;
      try { await fs.promises.rm(path.join(staging, entry.name), { recursive:true, force:true }); removed += 1; } catch (_) {}
    }
    return removed;
  }
  ...
  async exportFile(connector, localPath, remotePath, options = {}) {
    ...
    await this.run(['copyto', localPath, target, '--checksum', '--retries', '3', '--low-level-retries', '10'], {
      timeoutMs:options.timeoutMs || 24 * 60 * 60 * 1000, signal:options.signal,
    });
