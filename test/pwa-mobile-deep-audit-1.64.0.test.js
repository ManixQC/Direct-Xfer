...
test('1.64.8 release and pwa337 are synchronized',()=>{
  assert.match(read('package.json'),/"version"\s*:\s*"1\.64\.8"/);
  for(const f of ['pwa/app.js','pwa/index.html','pwa/sw.js','pwa/theme-init.js','pwa/admin-advanced.js','pwa/mobile-intelligence.js']) assert.match(read(f),/1\.64\.8|pwa337|v=337/);
...
test('shortcut manifests use the current pwa337 cache buster on app and login paths',()=>{
  assert.match(app,/direct-xfer-pwa\.webmanifest'[\s\S]*?\+ '\?v=337'/);
  assert.match(read('pwa/login.html'),/direct-xfer-pwa\.webmanifest\?v=337/);
});
