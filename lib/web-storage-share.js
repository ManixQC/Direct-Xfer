...
function createWebStorageShareTools(options = {}) {
  ...
  async function statCore(share, relative = '', statOptions = {}) {
    const meta = shareMeta(share);
    const full = joinedPath(share, relative);
    ...
    try { value = await storageConnectorService.stat(meta, full); }
      ...
      if (generation === cacheGeneration) statCache.delete(key);
      throw error;
    }
  ...
  async function stat(share, relative = '', statOptions = {}) {
    return withActiveOperation(() => statCore(share, relative, statOptions));
  }

  async function listCore(share, relative = '') {
    const meta = shareMeta(share);
    const full = joinedPath(share, relative);
    if (!meta || full === null) throw Object.assign(new Error('invalid-web-storage-share'), { code:'invalid-share' });
  ...
  async function list(share, relative = '') {
    return withActiveOperation(() => listCore(share, relative));
  }
  ...
  async function walkFiles(share, options = {}) {
    ...
    while (queue.length && files.length < maxFiles) {
      ...
      const rows = await listCore(share, current.rel);
  ...
  return {
    shareMeta, importMeta, joinedPath, relativePath, stat, list, walkFiles, invalidate,
    etag, parseRange, clearCache, isBusyForStateReplacement,
  };
}
