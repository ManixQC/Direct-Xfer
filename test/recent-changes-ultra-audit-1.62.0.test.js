...
test('1.62.0 release metadata is synchronized across Node, PWA, launcher, host and installer', () => {
  const pkg = JSON.parse(read('package.json'));
  const lock = JSON.parse(read('package-lock.json'));
  assert.equal(pkg.version, '1.62.0');
  assert.equal(lock.version, '1.62.0');
  assert.equal(lock.packages[''].version, '1.62.0');
  assert.match(read('pwa', 'app.js'), /APP_VERSION = '1\.62\.0'/);
  assert.match(read('pwa', 'app.js'), /APP_BUILD = '2026\.08\.15-pwa304'/);
  assert.match(read('pwa', 'sw.js'), /VERSION = '2026\.08\.15-pwa304'/);
  assert.match(read('pwa', 'index.html'), /v1\.62\.0 · pwa304/);
  assert.match(launcher, /AppVersion = "1\.62\.0"/);
  assert.match(launcher, /RuntimeAppBuild = "1\.62\.0-launcher41-csharp"/);
  assert.match(host, /HostVersion = "1\.62\.0-serverhost14-csharp"/);
  assert.match(workflow, /DX_VERSION: '1\.62\.0'/);
  assert.match(workflow, /DX_RUNTIME_BUILD: '1\.62\.0-launcher41-csharp'/);
  assert.match(workflow, /DX_SERVER_HOST_BUILD: '1\.62\.0-serverhost14-csharp'/);
  assert.match(iss, /#define AppVersion "1\.62\.0"/);
});
...
test('PWA and admin notification assets are cache-busted consistently while the favicon remains v269', () => {
  ...
  assert.match(read('pwa', 'index.html'), /app\.js\?v=288/);
  assert.match(read('pwa', 'sw.js'), /app\.css\?v=274/);
  assert.match(read('pwa', 'sw.js'), /app\.js\?v=288/);
  assert.match(read('public', 'index.html'), /style\.css\?v=286/);
  assert.match(read('public', 'index.html'), /app\.js\?v=291/);
