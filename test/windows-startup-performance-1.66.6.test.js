...
test('1.67.5 launcher defers optional-component housekeeping until backend startup completes', () => {
...
test('1.67.5 ServerHost validates app runtime and Node concurrently', () => {
...
test('1.67.5 Node hashing uses a large sequential buffer and pinned private Node skips redundant version process', () => {
...
test('1.67.5 search cache hydration is deferred while transfer-log trimming stays race-safe before listen', () => {
...
test('1.67.5 internal Windows build identifiers reflect startup optimization revision', () => {
  assert.match(launcher, /RuntimeAppBuild = "1\.67\.5-launcher92-csharp"/);
  assert.match(launcher, /ServerHostBuild = "1\.67\.5-serverhost65-csharp"/);
  assert.match(host, /RuntimeAppBuild = "1\.67\.5-launcher92-csharp"/);
  assert.match(host, /HostVersion = "1\.67\.5-serverhost65-csharp"/);
});
