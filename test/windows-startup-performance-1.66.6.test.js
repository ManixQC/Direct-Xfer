...
test('1.67.7 launcher defers optional-component housekeeping until backend startup completes', () => {
...
test('1.67.7 ServerHost validates app runtime and Node concurrently', () => {
...
test('1.67.7 Node hashing uses a large sequential buffer and pinned private Node skips redundant version process', () => {
...
test('1.67.7 search cache hydration is deferred while transfer-log trimming stays race-safe before listen', () => {
...
test('1.67.7 internal Windows build identifiers reflect startup optimization revision', () => {
  assert.match(launcher, /RuntimeAppBuild = "1\.67\.7-launcher94-csharp"/);
  assert.match(launcher, /ServerHostBuild = "1\.67\.7-serverhost67-csharp"/);
  assert.match(host, /RuntimeAppBuild = "1\.67\.7-launcher94-csharp"/);
  assert.match(host, /HostVersion = "1\.67\.7-serverhost67-csharp"/);
});
