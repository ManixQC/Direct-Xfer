# Direct-Xfer 1.71.48 — ASVS L3 release evidence

> 1.71.48 is a targeted dependency/container-security maintenance release. Express remains on 4.22.2; npm `overrides` forces transitive `qs` to 6.16.0, closing CVE-2026-82562 / GHSA-x5fp-wj9c-mxmx and CVE-2026-82417 / GHSA-4mjr-xmp4-gh2g without an Express 5 migration. Docker rclone is pinned to security release v1.75.1, built with Go 1.26.6 and verified `golang.org/x/crypto` v0.56.0, closing CVE-2026-56854; the container additionally raises grpc-go to the upstream CVE-2026-84445 security-fix version. Windows optional rclone stays on the signed upstream v1.75.1 archive; CVE-2026-84445 is an xDS-server flaw and Direct-Xfer does not expose rclone as a gRPC/xDS server. The 1.71.42 SignPath/CSP/ZAP/provenance controls remain the audited baseline.
...
| Security inventory | PASS | Regenerated for 1.71.48; 983 inventory entries |
...
| CycloneDX SBOM + provenance | PASS | Root component synchronized to Direct-Xfer 1.71.48 and transitive qs synchronized to 6.16.0; Windows provenance job validates the SBOM, emits build-provenance attestations for launcher, ServerHost, installer, the SHA-256 release manifest and an exact `git archive` source package, then binds the npm/source CycloneDX SBOM only to that source package so the SBOM subject accurately matches what it describes |
