...
const ROUTE_SERVICE_EXPORTS = Object.freeze({
  ...
  webauthn: Object.freeze([
    ...
    'passkeyDeviceIds', 'passkeyTransports', 'pruneWebauthnChallenges',
    ...
    get(_target, property) {
      ...
      if (!deferred.has(property)) {
        deferred.set(property, (...args) => {
          ...
          const operation = current && current[property];
          if (typeof operation !== 'function') {
            throw new Error(`pwa-service-not-ready:${name}.${property}`);
          }
          return operation.apply(current, args);
        });
      }
