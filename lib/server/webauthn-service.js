...
function phantomAllowCredentials(username, count = LOGIN_ALLOW_CREDENTIALS_COUNT, offset = 0) {
  ...
  for (let index = 0; index < wanted; index += 1) {
    const id = crypto.createHmac('sha256', phantomCredentialSecret)
      .update(`webauthn-phantom:${normalized}:${start + index}`)
      .digest();
...
function freshPasskeyManagementAccount(req, res) {
  ...
  // The first passkey can be bootstrapped from a freshly authenticated password/TOTP
  // session. Once any passkey exists, L3 factor management itself requires a recent
  // phishing-resistant ceremony so a stolen bootstrap session cannot add/remove keys.
  if (asvsL3Mode && accountPasskeys(acc).length > 0) {
