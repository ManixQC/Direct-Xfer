...
test('pwa465 loads and precaches the mobile intelligence module as a public shell asset', () => {
  for (const file of ['pwa/app.js','pwa/index.html','pwa/sw.js','pwa/theme-init.js','pwa/admin-advanced.js']) {
    assert.match(read(file), /pwa465|v=446/);
  }
  assert.match(index, /mobile-intelligence\.js\?v=446/);
  assert.match(sw, /\/app\/mobile-intelligence\.js\?v=446/);
...
test('fresh unauthenticated PWA bootstrap can fetch the new module and manifests', { timeout:25000 }, async () => {
  ...
  try {
    ...
    const mod = await fetch(base + '/app/mobile-intelligence.js?v=446');
    ...
    for (const name of ['manifest.webmanifest','manifest-en.webmanifest','manifest-es.webmanifest']) {
      const r = await fetch(base + '/app/' + name + '?v=446');
