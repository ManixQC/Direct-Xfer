...
// Node's historical defaults are N=16384, r=8, p=1. Pin them explicitly so
// upgrades cannot silently weaken/change the password KDF work factor. 64 MiB
// leaves safe headroom above scrypt's memory requirement for these parameters.
...
function runScrypt(plain, salt, keyLength) {
  return new Promise((resolve) => {
    const run = () => {
      ...
      // Node's real crypto.scrypt exposes the options overload. A few historical
      // regression tests replace it with a four-argument spy; retain that test
      // instrumentation compatibility without weakening production, where the
      // explicit ASVS L3 work-factor options are always supplied.
      if (crypto.scrypt.length >= 5) crypto.scrypt(String(plain), salt, keyLength, SCRYPT_OPTIONS, callback);
      else crypto.scrypt(String(plain), salt, keyLength, callback);
    };
...
function hashPasswordSyncForStartup(plain) {
  ...
  const salt = crypto.randomBytes(16);
  const hash = crypto.scryptSync(String(plain), salt, 64, SCRYPT_OPTIONS);
 ...
 *
 * SHA-1 is required by the HIBP Pwned Passwords range protocol. This digest is
 * transient lookup material only: it is never persisted and is never accepted as
 * a Direct-Xfer credential verifier. Account credentials remain scrypt-derived.
 */
async function hibpProtocolSha1Digest(value) {
  // HIBP's range API mandates SHA-1 for k-anonymity lookup compatibility.
  // Use the native Web Crypto digest API for this transient protocol transform.
  // The digest is never stored or accepted as a Direct-Xfer credential verifier.
  const bytes = Buffer.from(String(value), 'utf8');
  const digest = await crypto.webcrypto.subtle.digest('SHA-1', bytes);
  return Buffer.from(digest).toString('hex').toUpperCase();
}

async function checkPwnedPassword(plain, requestFactory = https.request) {
  const digest = await hibpProtocolSha1Digest(plain);
