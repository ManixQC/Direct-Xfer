...
test('1.59.1 hardening remains present after the 1.59.5 ServerHost split', () => {
  assert.match(launcher, /AssemblyVersion\("1\.59\.5\.0"\)/);
  assert.match(host, /AssemblyVersion\("1\.59\.5\.0"\)/);
  assert.match(launcher, /RuntimeAppBuild = "1\.59\.5-launcher31-csharp"/);
  assert.match(host, /RuntimeAppBuild = "1\.59\.5-launcher31-csharp"/);
  assert.match(workflow, /DX_VERSION: '1\.59\.5'/);
  assert.match(iss, /#define AppVersion "1\.59\.5"/);
});

test('PWA cache generation is advanced consistently for 1.59.5', () => {
...
test('all ServerHost critical runtime hashes match actual normalized 1.59.5 files', () => {
...
test('installer upgrade and GitHub release gates remain hardened', () => {
  ...
  assert.match(workflow, /recent-changes-ultra-audit-1\.59\.5\.test\.js/);
  assert.match(workflow, /windows-server-host-split-1\.59\.5\.test\.js/);
});
