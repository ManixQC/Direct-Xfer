...
test('1.61.1 release metadata is synchronized across Node, PWA, launcher, host and installer', () => {
  ...
  assert.match(launcher, /AppVersion = "1\.61\.1"/);
  assert.match(launcher, /RuntimeAppBuild = "1\.61\.1-launcher40-csharp"/);
  assert.match(host, /HostVersion = "1\.61\.1-serverhost13-csharp"/);
  assert.match(workflow, /DX_VERSION: '1\.61\.1'/);
  assert.match(workflow, /DX_RUNTIME_BUILD: '1\.61\.1-launcher40-csharp'/);
  assert.match(workflow, /DX_SERVER_HOST_BUILD: '1\.61\.1-serverhost13-csharp'/);
  assert.match(iss, /#define AppVersion "1\.61\.1"/);
});
...
test('PWA and admin notification assets are cache-busted consistently while the favicon remains v269', () => {
  ...
  assert.match(read('pwa', 'index.html'), /app\.js\?v=286/);
  assert.match(read('pwa', 'sw.js'), /app\.css\?v=274/);
  assert.match(read('pwa', 'sw.js'), /app\.js\?v=286/);
  assert.match(read('public', 'index.html'), /style\.css\?v=284/);
  assert.match(read('public', 'index.html'), /app\.js\?v=288/);
