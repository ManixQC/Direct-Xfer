...
test('1.67.6 release and pwa365 are synchronized',()=>{
  assert.match(read('package.json'),/"version"\s*:\s*"1\.67\.6"/);
  for(const f of ['pwa/app.js','pwa/index.html','pwa/sw.js','pwa/theme-init.js','pwa/admin-advanced.js','pwa/mobile-intelligence.js']) assert.match(read(f),/1\.67\.6|pwa365|v=365/);
...
test('shortcut manifests use the current pwa365 cache buster on app and login paths',()=>{
  assert.match(app,/direct-xfer-pwa\.webmanifest'[\s\S]*?\+ '\?v=365'/);
  assert.match(read('pwa/login.html'),/direct-xfer-pwa\.webmanifest\?v=365/);
});
