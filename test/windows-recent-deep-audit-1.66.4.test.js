...
<<<<<<< HEAD
test('1.71.3 bump advances the PWA cache generation so changed release metadata is not served from pwa353', () => {
  ...
  for (const rel of ['pwa/app.js', 'pwa/index.html', 'pwa/sw.js', 'pwa/theme-init.js', 'pwa/admin-advanced.js', 'pwa/mobile-intelligence.js']) {
    ...
    assert.match(source, /1\.71\.3|pwa466|v=447/);
=======
test('1.71.4 bump advances the PWA cache generation so changed release metadata is not served from pwa353', () => {
  assert.match(read('package.json'), /"version"\s*:\s*"1\.71\.4"/);
  for (const rel of ['pwa/app.js', 'pwa/index.html', 'pwa/sw.js', 'pwa/theme-init.js', 'pwa/admin-advanced.js', 'pwa/mobile-intelligence.js']) {
    const source = read(rel);
    assert.match(source, /1\.71\.4|pwa467|v=448/);
>>>>>>> eb50626 (v1.71.4)
