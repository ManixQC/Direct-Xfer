...
function webauthnRp(req) {
  const host = String(req && req.headers && req.headers.host || '').split(',')[0].trim() || 'localhost';
  const proto = req && req.protocol === 'https' ? 'https' : 'http';
  let requestUrl = null;
  try { requestUrl = new URL(`${proto}://${host}`); } catch (_) {}

  // Windows launches the local administrator UI on localhost so WebAuthn can use
  // the standards-defined loopback origin. PUBLIC_URL may still be configured for
  // externally shared links; it must not replace the browser's localhost RP/origin
  // or navigator.credentials.get() fails before Windows Hello can even prompt.
  if (requestUrl && String(requestUrl.hostname || '').toLowerCase() === 'localhost') {
    return { id: 'localhost', origin: requestUrl.origin, name: APP_NAME };
  }

  if (PUBLIC_URL) {
    try {
      const u = new URL(PUBLIC_URL);
      return { id: u.hostname, origin: u.origin, name: APP_NAME };
    } catch (_) {}
  }
  if (requestUrl) return { id: requestUrl.hostname, origin: requestUrl.origin, name: APP_NAME };
