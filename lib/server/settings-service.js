...
const { isAsvsL3OutboundUrlAllowed } = require('./asvs-l3-policy');
...
function createSettingsService(deps = {}) {
  const {
    ...
    asvsL3EgressAllowlist: ASVS_L3_EGRESS_ALLOWLIST = '',
    tlsCert: TLS_CERT = '',
    tlsKey: TLS_KEY = '',
  ...
  const DEFAULT_SETTINGS = {
    ...
    tokenBytes: 24, // random bytes for share/recipient link tokens (>=16 in ASVS L3)
  ...
  function settingsForClient(req, lite) {
    ...
    const client = {
      ...
      dataEncrypted: ASVS_L3_MODE === true || !!DATA_KEY,
  ...
  function computeSettingsPatch(body) {
    ...
    if (typeof body.webhookUrl === 'string') {
      ...
      if (u && ASVS_L3_MODE === true && !isAsvsL3OutboundUrlAllowed(u, { enabled:true, allowlist:ASVS_L3_EGRESS_ALLOWLIST })) return { error:'asvs-l3-egress-denied' };
    ...
    if (typeof body.tlsLocalCa === 'boolean') {
      if (ASVS_L3_MODE && body.tlsLocalCa) return { error:'asvs-l3-external-tls-required' };
      if (!tlsManagedByEnvironment()) {
        patch.tlsLocalCa = ASVS_L3_MODE ? false : body.tlsLocalCa;
        patch.tlsSelfSigned = false; // migrate away from the legacy 1.57.3 flag
      }
    } else if (typeof body.tlsSelfSigned === 'boolean') {
      if (ASVS_L3_MODE && body.tlsSelfSigned) return { error:'asvs-l3-external-tls-required' };
      if (!tlsManagedByEnvironment()) {
        patch.tlsLocalCa = ASVS_L3_MODE ? false : body.tlsSelfSigned;
        patch.tlsSelfSigned = false;
      }
    }
    if (body.tokenBytes !== undefined) patch.tokenBytes = clampNum(body.tokenBytes, ASVS_L3_MODE ? 16 : 12, 48, 24);
