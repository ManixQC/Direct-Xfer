...
function createOcrService(deps = {}) {
  ...
  function loadCacheSync() {
    try {
      const parsed = deserializeOcrCache(fs.readFileSync(CACHE_FILE, 'utf8'));
      ...
      if (e && e.code === 'ASVS_L3_PLAINTEXT_CACHE_REJECTED') try { fs.unlinkSync(CACHE_FILE); } catch (_) {}
    }
  }

  async function loadCacheDeferred(expectedGeneration, expectedEpoch, isCurrent) {
    try {
      const parsed = deserializeOcrCache(await fs.promises.readFile(CACHE_FILE, 'utf8'));
      ...
      if (!e || e.code !== 'ENOENT') console.warn('[search-ocr] cache could not be loaded; OCR entries will be rebuilt:', String((e && e.message) || e));
      if (e && e.code === 'ASVS_L3_PLAINTEXT_CACHE_REJECTED') try { await fs.promises.unlink(CACHE_FILE); } catch (_) {}
      return false;
    }
  }
