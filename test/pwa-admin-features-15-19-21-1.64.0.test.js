...
test('PWA build pwa483 loads and precaches audit/connectors administration module', () => {
  ...
  assert.match(theme, /admin-audit-connectors\.js\?v=464/);
  assert.match(sw, /admin-audit-connectors\.js\?v=464/);
...
test('real server verifies signed audit and imports a nested remote file server-to-server', { timeout:35000 }, async () => {
  ...
  try {
    ...
    const asset = await fetch(base + '/app/admin-audit-connectors.js?v=464');
