...
function verifierPayload(raw) {
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
      ...
      if (verifier && scope) return { verifier, scope };
    }
  } catch {}
  ...
  return { verifier:text, scope:LEGACY_SCOPE };
}
...
function html(title, message, status = 200, close = false) {
...
async function handleCreateSession(request, env) {
  ...
  await env.DB.prepare(`INSERT INTO sessions
    ...
    .bind(id, await sha256(pollToken), await sha256(state), await encryptText(env, JSON.stringify({ verifier, scope:requestedScope })), version, origin, now, now, now + SESSION_TTL_MS)
    .run();
  ...
  auth.searchParams.set('scope', requestedScope);
  auth.searchParams.set('access_type', 'offline');
  auth.searchParams.set('prompt', 'consent');
  auth.searchParams.set('state', state);
  auth.searchParams.set('code_challenge', challenge);
  auth.searchParams.set('code_challenge_method', 'S256');
  return json({ id, pollToken, authUrl:auth.toString(), scope:requestedScope, expiresAt:now + SESSION_TTL_MS }, 201);
}
