# Direct-Xfer 1.71.47 — ASVS L3 release evidence

> 1.71.47 is a targeted dependency/container-security maintenance release. Express remains on 4.22.2; npm `overrides` forces transitive `qs` to 6.16.0, closing CVE-2026-82562 / GHSA-x5fp-wj9c-mxmx and CVE-2026-82417 / GHSA-4mjr-xmp4-gh2g without an Express 5 migration. Docker rclone is pinned to security release v1.75.1, built with Go 1.26.6 and verified `golang.org/x/crypto` v0.56.0, closing CVE-2026-56854. Windows optional rclone uses the same v1.75.1 release. The 1.71.42 SignPath/CSP/ZAP/provenance controls remain the audited baseline.
...
| Security inventory | PASS | Regenerated for 1.71.47; 983 inventory entries |
...
| CycloneDX SBOM + provenance | PASS | Root component synchronized to Direct-Xfer 1.71.47 and transitive qs synchronized to 6.16.0; Windows provenance job validates the SBOM, emits build-provenance attestations for launcher, ServerHost, installer, the SHA-256 release manifest and an exact `git archive` source package, then binds the npm/source CycloneDX SBOM only to that source package so the SBOM subject accurately matches what it describes |
...
The expanded 1.71.47 maintenance gate executed 124 targeted release/security tests with zero failures or skips. The deep-audit follow-up hardened the CodeQL `js/clear-text-logging` fix further: account-bootstrap warnings now resolve an internal allowlisted warning code to a fixed diagnostic catalog, so callers cannot accidentally forward `error`, `error.message`, usernames, paths, environment-derived runtime data, or other dynamic values to the logger. The audit also corrected a stale historical account-service regression that still required randomized `owner-*` identities in normal compatibility mode; compatibility mode now explicitly tests the stable `admin` bootstrap while a separate ASVS L3 case continues to require a randomized owner identity. The PWA is synchronized to `pwa510`, cache generation `v=490` and build `2026.09.08-pwa510`, closing the stale Windows CI expectation shown by the failed build. The embedded OAuth Worker asset is byte-identical to its canonical source and the current owner-identity compatibility behavior is reflected by the ASVS regressions. In the clean source tree, the ASVS wildcard gate executes 89 tests with 88 passing and the sole failure being the `express`-dependent HTTP/CSV test; dependency-backed CI must reconfirm the complete ASVS gate after `npm ci`. The gate also locks the reviewed Dependabot GitHub Actions SHAs. Express stays at 4.22.2 and `qs` is forced to 6.16.0; dependency-backed CI exercises both the bracket-key/comma `arrayLimit` regression and the attacker-controlled `constructor.isBuffer` parse→stringify regression. Docker rclone is pinned to v1.75.1 / Go 1.26.6 and the Dockerfile verifies embedded `golang.org/x/crypto` v0.56.0 plus `golang.org/x/image` v0.45.0 before publishing the binary; Windows optional rclone is pinned to the same release and verified archive hash. Static and PARTIAL audits remain green. A full dependency-backed `npm ci` + `npm test` run remains required in CI before publishing binaries because the source archive intentionally omits `node_modules`.

## Commands/gates used

```text
node --test test/account-logging-security-1.71.47.test.js test/github-actions-dependency-pins-1.71.47.test.js test/qs-array-limit-security-1.71.47.test.js test/dependency-security-floors-1.71.8.test.js test/release-maintenance.test.js test/github-security-workflows.test.js test/codeql-security-regressions-1.70.22.test.js test/signpath-foundation-pipeline-1.71.4.test.js test/windows-latest-deep-audit-1.66.6.test.js test/windows-recent-deep-audit-1.66.4.test.js test/project-reconstruction-1.64.0.test.js test/oauth-broker-public-deep-audit-1.67.25.test.js test/pwa-mobile-deep-audit-1.64.0.test.js test/pwa-system-health-bottom-nav-1.64.4.test.js test/docker-scout-go-runtime-hardening-1.70.22.test.js test/docker-scout-go-runtime-hardening-deep-audit-1.70.22.test.js test/windows-modern-dotnet-1.64.10.test.js
