 ...
 */
function hibpProtocolSha1Digest(value) {
  // HIBP's range API mandates SHA-1 for k-anonymity lookup compatibility.
  // This transient digest is never stored or used as a Direct-Xfer credential verifier.
  return crypto.createHash('sha1').update(Buffer.from(String(value), 'utf8')).digest('hex').toUpperCase(); // lgtm[js/insufficient-password-hash]
}

function checkPwnedPassword(plain, requestFactory = https.request) {
  const digest = hibpProtocolSha1Digest(plain);
