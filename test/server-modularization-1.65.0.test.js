  ...
  try {
    global.fetch = async () => ({
      ...
      text: async () => JSON.stringify({ results: [
        ...
        { name:'1.66.4', digest:'sha256:new' },
        { name:'1.65.0', digest:'sha256:old' },
      ] }),
    });
    const svc = createNetworkServices({
      net: require('node:net'), os: require('node:os'), LOCAL_IP:'', APP_VERSION:'1.66.3', UPDATE_REPO:'owner/repo', UPDATE_TAG:'latest',
    ...
    assert.equal(live.latest, '1.66.4');
...
test('backup service serializes the current restored state instead of the empty proxy target', () => {
  ...
  try {
    ...
    const svc = createBackupService({
      ...
      APP_NAME:'Direct-Xfer', APP_VERSION:'1.66.3', getState:()=>current,
