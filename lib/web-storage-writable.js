...
function connectorStatus(error) {
  ...
  if (code === 'rclone-unavailable') return 503;
  if (code === 'remote-not-found') return 404;
  if (code === 'read-only') return 409;
  if (code === 'connector-timeout' || code === 'connector-cancelled') return 503;
  if (code === 'EINVAL' || code === 'invalid-share') return 400;
  return 502;
}
...
function createWebStorageWritableTools(options = {}) {
  ...
  async function exists(share, relative) {
    ...
    catch (error) { if (String(error && error.code) === 'remote-not-found' || /not found/i.test(String(error && error.message || ''))) return null; throw error; }
  }
  ...
  return { writableMeta, exists, reserveUnique, publishFile, mkdir, remove, metrics, trackPublished, releaseTracked, sanitizeTracked };
}
