...
function createServerConfig(options = {}) {
  ...
  // ASVS Level 3 enforcement profile. Disabled only for backward-compatible/local
  // deployments; a deployment claiming L3 must enable it and satisfy the startup
  // prerequisites validated by asvs-l3-policy.js.
  const ASVS_L3_MODE = bool(env.ASVS_L3_MODE);
  const ASVS_L3_MAX_FILES_PER_SENDER = Math.max(1, int(env.ASVS_L3_MAX_FILES_PER_SENDER, 1000));
  const ASVS_L3_MAX_BYTES_PER_SENDER = Math.max(1024 * 1024, int(env.ASVS_L3_MAX_BYTES_PER_SENDER, 20 * 1024 ** 3));
  const ASVS_L3_EGRESS_ALLOWLIST = String(env.ASVS_L3_EGRESS_ALLOWLIST || '').trim();
  const ASVS_L3_CRYPTO_PROVIDER = String(env.ASVS_L3_CRYPTO_PROVIDER || '').trim().toLowerCase();
  const ASVS_L3_CRYPTO_COMMAND = String(env.ASVS_L3_CRYPTO_COMMAND || '').trim();
  const ASVS_L3_EVIDENCE_FILE = String(env.ASVS_L3_EVIDENCE_FILE || '').trim();
  const ASVS_L3_EVIDENCE_PUBLIC_KEY = String(env.ASVS_L3_EVIDENCE_PUBLIC_KEY || '').replace(/\\n/g, '\n').trim();
  const ASVS_L3_EVIDENCE_PUBLIC_KEY_FILE = String(env.ASVS_L3_EVIDENCE_PUBLIC_KEY_FILE || '').trim();
  const ASVS_L3_HARDWARE_AAGUIDS = String(env.ASVS_L3_HARDWARE_AAGUIDS || '').trim();
  const ASVS_L3_ATTESTATION_ROOT_SHA256 = String(env.ASVS_L3_ATTESTATION_ROOT_SHA256 || '').trim();
  const ASVS_L3_ATTESTATION_ROOT_FILES = String(env.ASVS_L3_ATTESTATION_ROOT_FILES || '').trim();
  const AUDIT_HMAC_KEY = String(env.AUDIT_HMAC_KEY || '').trim();
  const AUDIT_SIGNING_PRIVATE_KEY = String(env.AUDIT_SIGNING_PRIVATE_KEY || '').trim();
  const AUDIT_SIGNING_PRIVATE_KEY_FILE = String(env.AUDIT_SIGNING_PRIVATE_KEY_FILE || '').trim();
  ...
  // ASVS V5.2.6: decoded-pixel-area cap. A small compressed file can decode to an
  // enormous image and exhaust memory/CPU in any decoder (server OCR, a viewer's
  // browser). 60 MP admits large legitimate photos while rejecting pixel floods.
  const IMAGE_MAX_PIXELS = Math.max(1_000_000, int(env.IMAGE_MAX_PIXELS, 60_000_000));
  ...
  const CLAMAV_SOCKET = String(env.CLAMAV_SOCKET || '').trim();
  const CLAMAV_TLS = bool(env.CLAMAV_TLS);
  const CLAMAV_TLS_SERVERNAME = String(env.CLAMAV_TLS_SERVERNAME || '').trim();
  const CLAMAV_TLS_CA_FILE = String(env.CLAMAV_TLS_CA_FILE || '').trim();
  const QUARANTINE_DIR = path.join(DATA_DIR, 'quarantine');
  const clamavEnabled = () => !!CLAMAV_HOST || !!CLAMAV_SOCKET || ASVS_L3_MODE;
  ...
  const groups = Object.freeze({
    ...
    security:Object.freeze({
      ...
      ASVS_L3_MODE, ASVS_L3_EGRESS_ALLOWLIST, ASVS_L3_CRYPTO_PROVIDER, ASVS_L3_CRYPTO_COMMAND, ASVS_L3_EVIDENCE_FILE, ASVS_L3_EVIDENCE_PUBLIC_KEY, ASVS_L3_EVIDENCE_PUBLIC_KEY_FILE, ASVS_L3_HARDWARE_AAGUIDS, ASVS_L3_ATTESTATION_ROOT_SHA256, ASVS_L3_ATTESTATION_ROOT_FILES,
      AUDIT_HMAC_KEY, AUDIT_SIGNING_PRIVATE_KEY, AUDIT_SIGNING_PRIVATE_KEY_FILE, AUDIT_REMOTE_URL,
    }),
    ...
    limits:Object.freeze({
      ...
      ASVS_L3_MAX_FILES_PER_SENDER, ASVS_L3_MAX_BYTES_PER_SENDER,
    ...
    connectors:Object.freeze({
      GOOGLE_OAUTH_BROKER_URL_ENV, RCLONE_BIN, CLAMAV_HOST, CLAMAV_PORT, CLAMAV_SOCKET, CLAMAV_TLS, CLAMAV_TLS_SERVERNAME, CLAMAV_TLS_CA_FILE, clamavEnabled,
    }),
  ...
  const config = {
    ...
    DATA_KEY, ASVS_L3_MODE, ASVS_L3_MAX_FILES_PER_SENDER, ASVS_L3_MAX_BYTES_PER_SENDER,
    ASVS_L3_EGRESS_ALLOWLIST, ASVS_L3_CRYPTO_PROVIDER, ASVS_L3_CRYPTO_COMMAND, ASVS_L3_EVIDENCE_FILE, ASVS_L3_EVIDENCE_PUBLIC_KEY, ASVS_L3_EVIDENCE_PUBLIC_KEY_FILE, ASVS_L3_HARDWARE_AAGUIDS, ASVS_L3_ATTESTATION_ROOT_SHA256, ASVS_L3_ATTESTATION_ROOT_FILES,
    AUDIT_HMAC_KEY, AUDIT_SIGNING_PRIVATE_KEY, AUDIT_SIGNING_PRIVATE_KEY_FILE, AUDIT_REMOTE_URL, AUDIT_REMOTE_TOKEN,
    GOOGLE_OAUTH_BROKER_URL_ENV, INBOX_DIR, PENDING_DIR,
    ...
    PHOTO_HISTORY_MAX, IMAGE_MAX_BYTES, IMAGE_MAX_PIXELS, THUMB_MAX_BYTES, MICRO_MAX_BYTES,
    CLAMAV_HOST, CLAMAV_PORT, CLAMAV_SOCKET, CLAMAV_TLS, CLAMAV_TLS_SERVERNAME, CLAMAV_TLS_CA_FILE, QUARANTINE_DIR, clamavEnabled, MAX_LOG_BYTES,
