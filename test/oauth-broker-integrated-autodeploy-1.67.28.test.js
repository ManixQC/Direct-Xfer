...
test('1.69.6 installed UI offers broker deployment without source files or Wrangler', () => {
  ...
  assert.match(adminStorage, /createOAuthBrokerDeploymentRoutes/);
...
test('1.69.6 Cloudflare token is kept server-side during the wizard and never returned to the browser', async () => {
...
test('1.69.6 reports the exact Cloudflare deployment stage instead of a generic failure', async () => {
...
test('1.69.6 validates Cloudflare token status before changing account resources', async () => {
  const originalFetch = global.fetch;
  let calls = 0;
  global.fetch = async (url) => {
    calls += 1;
