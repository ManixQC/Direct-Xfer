...
test('1.71.13 keeps the audited dependency maintenance baseline', () => {
  assert.equal(pkg.version, '1.71.13');
...
test('1.71.13 PWA release metadata advances atomically', () => {
  for (const rel of ['pwa/app.js', 'pwa/sw.js', 'pwa/theme-init.js', 'pwa/index.html']) {
    const source = read(rel);
    assert.match(source, /1\.71\.13|pwa476|v=457/);
    assert.doesNotMatch(source, /1\.71\.12|pwa475|v=456/);
  }
  assert.match(read('pwa/app.js'), /APP_BUILD = '2026\.08\.26-pwa476'/);
  assert.match(read('pwa/sw.js'), /VERSION = '2026\.08\.26-pwa476'/);
});

test('1.71.13 Tesseract source acquisition targets the exact annotated release tag ref', () => {
