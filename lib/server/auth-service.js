...
const DEFAULT_TEMPORARY_PASSWORD_TTL_MS = 15 * 60 * 1000;
...
function createAuthService(options = {}) {
  ...
  // Production composition uses the standard account hasher and therefore the
  // full ASVS password policy. Tests/embedders that intentionally inject a fake
  // hasher may inject their own passwordPolicy as well; absent that, avoid making
  // a synthetic hasher depend on an external breach-corpus request.
  const enforcePasswordPolicy = typeof passwordPolicy === 'function'
    ? passwordPolicy
    : (passwordHasher === hashPassword ? validateAccountPassword : async () => ({ ok:true }));
  ...
  function enableTotpFor(account, input) {
    if (asvsL3Mode) return { ok:false, error:'l3-hardware-passkey-required' };
    const twoFactor = account && account.totp;
    if (!twoFactor || !twoFactor.secret) return { ok: false, error: 'no-setup' };
    if (twoFactor.enabled) return { ok: true, alreadyEnabled: true };
    const matchedCounter = matchingTotpCounter(twoFactor.secret, input, 0);
    if (matchedCounter === null) return { ok: false, error: 'invalid-code' };

    const previousEnabled = twoFactor.enabled;
    const previousCounter = twoFactor.lastCounter;
    twoFactor.enabled = true;
    // The enrollment proof is itself a successful use of this OTP. Persist its
    // counter atomically with enabling so it cannot be reused immediately to log
    // in during the remainder of the same 30-second step (ASVS v6.5.1).
    twoFactor.lastCounter = matchedCounter;
    if (persistNow()) return { ok: true };

    twoFactor.enabled = previousEnabled;
    if (previousCounter === undefined) delete twoFactor.lastCounter;
    else twoFactor.lastCounter = previousCounter;
  ...
  async function verifyTotpOrRecoveryFor(account, input) {
    ...
    if (/^\d{6}$/.test(normalized)) {
      const result = consumeTotpFor(account, normalized);
      return { ...result, method:'totp' };
    }
    ...
    if (!/^(?:[0-9a-f]{10}|[0-9a-f]{32})$/.test(candidate)) return { ok: false }; // legacy 40-bit + ASVS L3 128-bit recovery codes
    ...
    for (const storedHash of codes.slice()) {
      ...
      if (persistNow()) return { ok: true, method:'recovery-code' };
  ...
  async function attemptLogin(req, res, username, password, totp) {
    ...
    try {
      ...
      if (twoFactorEnabledFor(account)) {
        if (!totp) {
          logAudit('login-2fa-required', { account, ip, authMethod:'password+second-factor' });
        ...
        if (!secondFactor.ok) {
          ...
          logAudit('login-2fa-fail', {
            ...
            authMethod: secondFactor.method ? `password+${secondFactor.method}` : 'password+second-factor',
          });
      ...
      // ASVS L3 requires a hardware-backed, phishing-resistant factor for the
      // administrator application. Password/TOTP remains available only as a
      // bootstrap path until the account has enrolled its first passkey; after
      // that, password login cannot mint a session in L3 mode.
      if (asvsL3Mode && Array.isArray(account.passkeys) && account.passkeys.length > 0) {
        logAudit('login-passkey-required', { account, ip, detail:'ASVS L3', authMethod:'password' });
        return { ok:false, passkeyRequired:true };
      }

      loginAttempts.delete(ip);
      account.lastLoginAt = Date.now();
      scheduleFlush();
      const successfulAuthMethod = twoFactorEnabledFor(account)
        ? `password+${secondFactorMethod || 'second-factor'}`
        : 'password';
      const session = createSession(req, res, account, { authMethod:successfulAuthMethod, phishingResistant:false });
      ...
      logAudit('login', {
        ...
        authMethod:successfulAuthMethod,
      });
  ...
  async function validateNewPassword(username, newPassword) {
    try {
      const result = await enforcePasswordPolicy(newPassword, { username });
      return result && typeof result === 'object' ? result : { ok:false, error:'password-policy-unavailable' };
    } catch (_) {
      return { ok:false, error:'password-policy-unavailable' };
    }
  }
  ...
  async function setAccountPassword(
    ...
    const hadPreviousExpiry = Object.hasOwn(account, 'bootstrapPasswordExpiresAt');
    const previousExpiry = account.bootstrapPasswordExpiresAt;
    if (expectedHash !== undefined && previousHash !== expectedHash) {
      return { ok: false, error: 'account-changed' };
    }

    const policy = await validateNewPassword(account.username || '', newPassword);
    if (!policy.ok) return { ok:false, error:policy.error || 'password-policy-unavailable' };

    // Policy checking can perform a bounded network lookup. Refuse stale work
    // before spending scrypt resources if the account changed while it was pending.
    const afterPolicy = getAccountById(account.id);
    if (afterPolicy !== account
        || afterPolicy.ah !== previousHash
        || afterPolicy.pwChanged !== previousPwChanged
        || (hadPreviousExpiry
          ? afterPolicy.bootstrapPasswordExpiresAt !== previousExpiry
          : Object.hasOwn(afterPolicy, 'bootstrapPasswordExpiresAt'))) {
      return { ok:false, error:'account-changed' };
    }
    ...
    if (currentAccount !== account
        ...
        || (hadPreviousExpiry
          ? currentAccount.bootstrapPasswordExpiresAt !== previousExpiry
          : Object.hasOwn(currentAccount, 'bootstrapPasswordExpiresAt'))) {
      return { ok: false, error: 'account-changed' };
    }
    ...
    if (account.pwChanged) delete account.bootstrapPasswordExpiresAt;
    else account.bootstrapPasswordExpiresAt = Date.now() + DEFAULT_TEMPORARY_PASSWORD_TTL_MS;
    if (persistNow()) return { ok: true, account };

    account.ah = previousHash;
    account.pwChanged = previousPwChanged;
    if (hadPreviousExpiry) account.bootstrapPasswordExpiresAt = previousExpiry;
    else delete account.bootstrapPasswordExpiresAt;
    return { ok: false, error: 'write-error' };
  }
  ...
  return {
    ...
    verifyTotp,
    enableTotpFor,
    twoFactorEnabledFor, l3TotpDisabled:asvsL3Mode === true,
    lockMs,
    cleanup,
    clearRuntimeState,
