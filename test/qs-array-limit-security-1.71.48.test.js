...
test('1.71.48 keeps Express on 4.22.2 and forces qs 6.16.0 for GHSA-x5fp-wj9c-mxmx and GHSA-4mjr-xmp4-gh2g', () => {
  assert.equal(pkg.version, '1.71.48');
...
test('1.71.48 exercises the bracket-key comma arrayLimit regression when dependencies are installed', () => {
  let qs;
  try {
    qs = require('qs');
  } catch (error) {
...
test('1.71.48 exercises the attacker-controlled isBuffer stringify regression when dependencies are installed', () => {
  let qs;
  try {
    qs = require('qs');
  } catch (error) {
