...
test('1.66.2 release and pwa352 are synchronized',()=>{
  assert.match(read('package.json'),/"version"\s*:\s*"1\.66\.2"/);
  for(const f of ['pwa/app.js','pwa/index.html','pwa/sw.js','pwa/theme-init.js','pwa/admin-advanced.js','pwa/mobile-intelligence.js']) assert.match(read(f),/1\.66\.2|pwa352|v=352/);
...
test('shortcut manifests use the current pwa352 cache buster on app and login paths',()=>{
  assert.match(app,/direct-xfer-pwa\.webmanifest'[\s\S]*?\+ '\?v=352'/);
  assert.match(read('pwa/login.html'),/direct-xfer-pwa\.webmanifest\?v=352/);
});
