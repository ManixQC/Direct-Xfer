...
test('device-only status lists all devices paired to its account', async()=>{
  ...
  for (const current of [phone, tablet]) {
    ...
    const statusUrl=`${base}/app/device/status?version=${encodeURIComponent('1.60.0')}&build=${encodeURIComponent('2026.08.15-pwa289')}`;
    ...
    assert.equal(currentDevice.appVersion,'1.60.0');
    assert.equal(currentDevice.appBuild,'2026.08.15-pwa289');
  }
});
