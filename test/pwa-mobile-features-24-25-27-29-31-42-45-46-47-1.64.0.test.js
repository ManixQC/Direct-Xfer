...
test('pwa346 loads and precaches the mobile intelligence module as a public shell asset', () => {
  for (const file of ['pwa/app.js','pwa/index.html','pwa/sw.js','pwa/theme-init.js','pwa/admin-advanced.js']) {
    assert.match(read(file), /pwa346|v=346/);
  }
  assert.match(index, /mobile-intelligence\.js\?v=346/);
  assert.match(sw, /\/app\/mobile-intelligence\.js\?v=346/);
...
test('fresh unauthenticated PWA bootstrap can fetch the new module and manifests', { timeout:25000 }, async () => {
  ...
  try {
    ...
    const mod = await fetch(base + '/app/mobile-intelligence.js?v=346');
    ...
    for (const name of ['manifest.webmanifest','manifest-en.webmanifest','manifest-es.webmanifest']) {
      const r = await fetch(base + '/app/' + name + '?v=346');
