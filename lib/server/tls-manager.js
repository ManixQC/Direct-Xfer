...
function generateForgeRsaKeyPair(bits=2048) {
...
function localTlsIdentities() {
  ...
  if (!wildcard) addIp(BIND);
    ...
    for (const entries of Object.values(os.networkInterfaces() || {})) {
      for (const item of entries || []) { if (item && !item.internal) addIp(item.address); }
    }
  }
...
function validateLocalCaCertificate(cert, key = null) {
  ...
  if (rsaPublicKeyBits(cert.publicKey) < 2048) throw new Error('CA RSA key is weaker than 2048 bits');
...
function validateLeafCertificate(cert, key, caCert, identities = null, renewalCheck = true, requireAki = true) {
  ...
  if (rsaPublicKeyBits(cert.publicKey) < 2048) throw new Error('server certificate RSA key is weaker than 2048 bits');
...
function ensureLocalServerCertificate(ca) {
  ...
  const keys = generateForgeRsaKeyPair(2048), cert = forge.pki.createCertificate();
  cert.publicKey = keys.publicKey; cert.serialNumber = randomCertSerial();
