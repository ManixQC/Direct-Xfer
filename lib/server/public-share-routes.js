...
// ASVS L3: public URL tokens are identifiers, never sufficient bearer
// credentials. Every externally accessible share must require an independent
// authorization gate (password or explicit access approval). This preserves the
// existing clean public URL model while preventing a token leaked through browser
// history, logs, screenshots, or referrals from granting access on its own.
if (ASVS_L3_MODE === true) downloadRouter.use((req, res, next) => {
  const match = publicPathShare(req);
  if (!match) return next();
  const s = match.share;
  if (s.pwHash || s.requestAccess) return next();
  const p = String(req.path || '');
  if (req.method !== 'GET' || /\/(?:list|upload|upload-status|dedupe|delete|remove|thread|message|folder)(?:\/|$)/.test(p)) {
    return res.status(403).json({ error:'l3-independent-auth-required' });
  }
  const lang = pickLang(req);
  const L = PUB[lang] || PUB.en;
...
function albumCollaborationPage(lang, album, members, invite, secret) {
  ...
  const cells = members.map((m) => {
    const safeToken = encodeURIComponent(String(m.token || ''));
    const full = '/i/' + safeToken + '/auto?w=1920';
    const thumb = '/i/' + safeToken + '/auto?w=480';
...
async function unlockHandler(req, res) {
  ...
  try {
    ...
    // Compatibility facade; modern scrypt hashes need no in-place upgrade.
