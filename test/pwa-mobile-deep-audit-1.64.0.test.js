...
test('1.70.23 release and pwa456 are synchronized',()=>{
  assert.match(read('package.json'),/"version"\s*:\s*"1\.70\.23"/);
  for(const f of ['pwa/app.js','pwa/index.html','pwa/sw.js','pwa/theme-init.js','pwa/admin-advanced.js','pwa/mobile-intelligence.js']) assert.match(read(f),/1\.70\.23|pwa456|v=441/);
...
test('shortcut manifests use the current pwa456 cache buster on app and login paths',()=>{
