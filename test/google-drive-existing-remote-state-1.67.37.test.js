...
test('1.68.0 reads only safe Google scope metadata from an existing rclone remote', async () => {
  const dir = tempDir();
...
test('1.68.0 Google OAuth start treats an existing remote as already connected rather than an OAuth error', () => {
...
test('1.68.0 connector UI does not show Waiting for Google for an existing configured remote', () => {
...
test('1.68.0 configuration page always reloads rclone remotes even before a Direct-Xfer connector record exists', () => {
  const app = read('public/app.js');
