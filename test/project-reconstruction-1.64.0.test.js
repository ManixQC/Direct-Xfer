...
test('Windows metadata targets 1.66.2 and current runtime hashes', () => {
  const launcher = read('windows-launcher/Program.cs');
  const host = read('windows-server-host/Program.cs');
  assert.match(launcher, /AppVersion = "1\.66\.2"/);
  assert.match(launcher, /RuntimeAppBuild = "1\.66\.2-launcher75-csharp"/);
  assert.match(host, /HostVersion = "1\.66\.2-serverhost49-csharp"/);
...
test('Windows GitHub Actions run name follows Direct-Xfer 1.66.2', () => {
  const workflow = read('.github/workflows/build-windows-csharp.yml');
  assert.match(workflow, /^run-name: v1\.66\.2$/m);
});
