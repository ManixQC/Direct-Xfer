...
test('1.68.1 launcher defers optional-component housekeeping until backend startup completes', () => {
...
test('1.68.1 ServerHost validates app runtime and Node concurrently', () => {
...
test('1.68.1 Node hashing uses a large sequential buffer and pinned private Node skips redundant version process', () => {
...
test('1.68.1 search cache hydration is deferred while transfer-log trimming stays race-safe before listen', () => {
...
test('1.68.1 internal Windows build identifiers reflect startup optimization revision', () => {
  assert.match(launcher, /RuntimeAppBuild = "1\.68\.1-launcher125-csharp"/);
  assert.match(launcher, /ServerHostBuild = "1\.68\.1-serverhost98-csharp"/);
  assert.match(host, /RuntimeAppBuild = "1\.68\.1-launcher125-csharp"/);
  assert.match(host, /HostVersion = "1\.68\.1-serverhost98-csharp"/);
});
