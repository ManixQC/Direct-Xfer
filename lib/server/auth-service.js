...
function createAuthService(options = {}) {
  ...
  function verifyTotp(secret, token, window = 1) {
    ...
    for (let offset = -window; offset <= window; offset += 1) {
      ...
      if (expected.length === candidate.length
          && crypto.timingSafeEqual(Buffer.from(expected), Buffer.from(candidate))) return true;
    }
    return false;
  }
  ...
  async function verifyTotpOrRecoveryFor(account, input) {
    ...
    if (verifyTotp(twoFactor.secret, input)) return { ok: true };

    const codes = Array.isArray(twoFactor.recovery) ? twoFactor.recovery : [];
    const candidate = String(input || '').replace(/\s/g, '').toLowerCase();
    // A wrong six-digit TOTP must not trigger the expensive recovery-code scrypt
    // loop. Recovery codes have the fixed hexadecimal format generated by DX.
