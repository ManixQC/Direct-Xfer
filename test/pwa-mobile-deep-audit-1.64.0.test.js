...
test('1.65.6 release and pwa346 are synchronized',()=>{
  assert.match(read('package.json'),/"version"\s*:\s*"1\.65\.6"/);
  for(const f of ['pwa/app.js','pwa/index.html','pwa/sw.js','pwa/theme-init.js','pwa/admin-advanced.js','pwa/mobile-intelligence.js']) assert.match(read(f),/1\.65\.6|pwa346|v=346/);
...
test('shortcut manifests use the current pwa346 cache buster on app and login paths',()=>{
  assert.match(app,/direct-xfer-pwa\.webmanifest'[\s\S]*?\+ '\?v=346'/);
  assert.match(read('pwa/login.html'),/direct-xfer-pwa\.webmanifest\?v=346/);
});
