...
function attachAdminAccountRoutes(deps = {}) {
  ...
  for (const [name, value] of Object.entries({
    ...
    if (typeof value !== 'function') throw new TypeError(`admin-account-routes requires authService.${name}()`);
  }
  const {
    destroySession,
  ...
  function sendAccountPasswordError(res, error) {
    const code = String(error || '');
    if (['too-short', 'password-too-long', 'password-common', 'password-contextual', 'password-breached'].includes(code)) {
      return res.status(400).json({ error:code });
    }
    if (['password-breach-check-unavailable', 'password-policy-unavailable'].includes(code)) {
      return res.status(503).json({ error:code });
    }
    return sendPasswordWorkError(res, code);
  }
  ...
  adminRouter.post('/password', async (req, res) => {
    ...
    if (!persisted.ok) {
      ...
      return sendAccountPasswordError(res, persisted.error);
    }
    clearOtherSessionsOfAccount(account.id, req.session.sid);
    auditReq(req, 'password-changed');
    addCenterNotification(account.id, 'auth-credential-changed', { category:'security', severity:'warning', detail:'Password changed', dedupeKey:`auth-change:password:${Date.now()}` });
    return res.json({ ok: true, persisted: true });
  });

  adminRouter.get('/2fa/status', (req, res) => {
    res.json({ enabled: twoFactorEnabledFor(currentAccount(req)), available:authService.l3TotpDisabled !== true });
  });
  adminRouter.post('/2fa/setup', async (req, res) => {
    if (authService.l3TotpDisabled === true) return res.status(409).json({ error:'l3-hardware-passkey-required' });
    const account = currentAccount(req);
    if (!account) return res.status(401).json({ error: 'not-authenticated' });
    ...
    const recoveryPlain = [];
    for (let i = 0; i < 8; i += 1) recoveryPlain.push(crypto.randomBytes(16).toString('hex')); // 128-bit recovery secrets
  ...
  adminRouter.post('/2fa/enable', (req, res) => {
    ...
    const enabled = enableTotpFor(account, String((req.body && req.body.code) || ''));
    if (!enabled.ok) {
      if (enabled.error === 'write-error') return res.status(503).json({ error: 'write-error' });
      return res.status(400).json({ error: enabled.error || 'invalid-code' });
    }
    // Factor changes invalidate sibling sessions so a stolen pre-MFA browser
    // session cannot survive enrollment and retain a weaker authentication state.
    clearOtherSessionsOfAccount(account.id, req.session.sid);
    auditReq(req, '2fa-enabled');
    addCenterNotification(account.id, 'auth-credential-changed', { category:'security', severity:'warning', detail:'TOTP factor enabled', dedupeKey:`auth-change:totp-enabled:${Date.now()}` });
  ...
  adminRouter.post('/2fa/disable', async (req, res) => {
    ...
    // Keep only the freshly password-verified session. All other sessions were
    // authenticated under a factor policy that has just changed.
    clearOtherSessionsOfAccount(account.id, req.session.sid);
    auditReq(req, '2fa-disabled');
    addCenterNotification(account.id, 'auth-credential-changed', { category:'security', severity:'critical', detail:'TOTP factor disabled', dedupeKey:`auth-change:totp-disabled:${Date.now()}` });
    return res.json({ ok: true });
  });
  ...
  adminRouter.post('/accounts', requireOwner, async (req, res) => {
    ...
    // Account creation previously bypassed the account password policy by calling
    // the raw scrypt helper directly. Check the breached/common/contextual policy
    // first, then revalidate owner authorization and username uniqueness after the
    // potentially asynchronous corpus lookup.
    const passwordPolicy = await validateNewPassword(username, password);
    if (!passwordPolicy.ok) return sendAccountPasswordError(res, passwordPolicy.error);
  ...
  adminRouter.post('/accounts/:id/password', requireOwner, async (req, res) => {
    ...
    if (account.role === 'owner' && adminPwFromEnv) return res.status(409).json({ error: 'env-managed' });
    // ASVS V6.4.6: an administrator may INITIATE reset, but must not choose the
    // user's password. Generate a high-entropy temporary bootstrap credential and
    // force the user to select their own password on the next authenticated use.
    ...
    if (!passwordUpdate.ok) {
      ...
      return sendAccountPasswordError(res, passwordUpdate.error);
    }
    if (!sessionService.isSessionActive(req.session.sid, ['owner'])) {
      return res.status(401).json({ error: 'not-authenticated' });
    }
    clearSessionsOfAccount(account.id);
    auditReq(req, 'password-reset', `user=${account.username}; system-generated temporary credential`);
    addCenterNotification(account.id, 'auth-credential-changed', { category:'security', severity:'critical', detail:'Password reset initiated by owner; all sessions revoked', dedupeKey:`auth-change:password-reset:${Date.now()}` });
