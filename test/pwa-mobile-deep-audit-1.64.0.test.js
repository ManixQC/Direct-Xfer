...
test('1.65.0 release and pwa340 are synchronized',()=>{
  assert.match(read('package.json'),/"version"\s*:\s*"1\.65\.0"/);
  for(const f of ['pwa/app.js','pwa/index.html','pwa/sw.js','pwa/theme-init.js','pwa/admin-advanced.js','pwa/mobile-intelligence.js']) assert.match(read(f),/1\.65\.0|pwa340|v=340/);
...
test('shortcut manifests use the current pwa340 cache buster on app and login paths',()=>{
  assert.match(app,/direct-xfer-pwa\.webmanifest'[\s\S]*?\+ '\?v=340'/);
  assert.match(read('pwa/login.html'),/direct-xfer-pwa\.webmanifest\?v=340/);
});
