...
test('PWA build pwa356 loads and precaches audit/connectors administration module', () => {
  ...
  assert.match(theme, /admin-audit-connectors\.js\?v=356/);
  assert.match(sw, /admin-audit-connectors\.js\?v=356/);
...
test('real server verifies signed audit and imports a nested remote file server-to-server', { timeout:35000 }, async () => {
  ...
  try {
    ...
    const asset = await fetch(base + '/app/admin-audit-connectors.js?v=356');
