# Direct-Xfer 1.71.29 — ASVS L3 release evidence

> 1.71.29 hardens the GitHub security pipeline after the first 1.71.28 runs. The rclone builder now tolerates transient Go proxy/checksum-service transport resets with bounded retry/backoff while keeping `GOSUMDB=sum.golang.org` and `go mod verify` fail-closed. OpenSSF Scorecard still runs and publishes its complete report, but governance/project-health checks such as `CodeReviewID` are filtered from the SARIF uploaded to GitHub Code Scanning; technical supply-chain security findings remain visible. PWA advances to pwa492 / cache-buster v=473.
...
| Security inventory | PASS | Regenerated for 1.71.29; 956 inventory entries |
...
| CycloneDX SBOM | PASS | Root component synchronized to Direct-Xfer 1.71.29 |
...
The 1.71.29 packaging pass executed 124 release/security-targeted tests with zero failures or skips, plus both repository-verifiable ASVS audits. The source archive intentionally excludes `node_modules`; real-server tests that require `express` therefore belong to the dependency-backed `npm ci` + `npm test` CI release gate rather than the source-only packaging pass.
