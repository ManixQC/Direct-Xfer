...
test('1.62.3 Windows server metadata is synchronized', () => {
  assert.match(host, /AppVersion = "1\.62\.3"/);
  assert.match(host, /RuntimeAppBuild = "1\.62\.3-launcher44-csharp"/);
  assert.match(host, /HostVersion = "1\.62\.3-serverhost17-csharp"/);
  assert.match(launcher, /RuntimeAppBuild = "1\.62\.3-launcher44-csharp"/);
  assert.match(launcher, /ServerHostBuild = "1\.62\.3-serverhost17-csharp"/);
...
test('all ServerHost critical runtime hashes match the exact normalized 1.62.3 runtime files', () => {
