...
test('1.68.2 installed UI offers broker deployment without source files or Wrangler', () => {
...
test('1.68.2 embedded Cloudflare assets stay byte-identical to the standalone broker', () => {
...
test('1.68.2 automatic deployment uses Cloudflare APIs directly and never shells out to Wrangler', () => {
...
test('1.68.2 Cloudflare token is kept server-side during the wizard and never returned to the browser', async () => {
  ...
  global.fetch = async (url, options = {}) => {
    ...
    if (u === 'https://api.cloudflare.com/client/v4/user/tokens/verify') return jsonResponse({ success:true, result:{ id:'b'.repeat(32), status:'active' } });
...
test('1.68.2 reports the exact Cloudflare deployment stage instead of a generic failure', async () => {
  const accountId = 'c'.repeat(32);
  const originalFetch = global.fetch;
  global.fetch = async (url) => {
    const u = String(url);
    if (u.endsWith('/user/tokens/verify')) return jsonResponse({ success:true, result:{ id:'d'.repeat(32), status:'active' } });
    if (u.endsWith(`/accounts/${accountId}/workers/subdomain`)) throw new TypeError('fetch failed: ENOTFOUND api.cloudflare.com');
    throw new Error(`unexpected fetch ${u}`);
  };
  try {
    const h = harness();
    const result = await h.call('/storage/oauth/broker-auto/prepare', {
      apiToken:'cf_test_token_abcdefghijklmnopqrstuvwxyz0123456789',
      accountId,
    });
    assert.equal(result.statusCode, 502);
    assert.equal(result.payload.error, 'cloudflare-network-error');
    assert.equal(result.payload.stage, 'workers-subdomain');
    assert.match(result.payload.detail, /ENOTFOUND|fetch failed/i);
  } finally { global.fetch = originalFetch; }
});

test('1.68.2 validates Cloudflare token status before changing account resources', async () => {
  const originalFetch = global.fetch;
  let calls = 0;
  global.fetch = async (url) => {
    calls += 1;
    const u = String(url);
    assert.match(u, /\/user\/tokens\/verify$/);
    return jsonResponse({ success:true, result:{ id:'e'.repeat(32), status:'disabled' } });
  };
  try {
    const h = harness();
    const result = await h.call('/storage/oauth/broker-auto/prepare', { apiToken:'cf_test_token_abcdefghijklmnopqrstuvwxyz0123456789' });
    assert.equal(result.statusCode, 403);
    assert.equal(result.payload.error, 'cloudflare-token-inactive');
    assert.equal(result.payload.stage, 'token-verify');
