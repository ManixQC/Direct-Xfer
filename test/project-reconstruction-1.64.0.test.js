...
test('Windows metadata keeps app 1.71.9 separate from stable Windows component identities', () => {
...
test('Windows GitHub Actions run name follows Direct-Xfer 1.71.9', () => {
  const workflow = read('.github/workflows/build-windows-csharp.yml');
  assert.match(workflow, /^run-name: v1\.71\.9$/m);
});
