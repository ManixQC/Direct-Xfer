...
function createWebStorageUploadHandler(options = {}) {
  ...
  return async function handleWebStorageUpload(req, res, s) {
    ...
    let failed = false, written = offset, reqEnded = false, writeFinished = false;
    const ws = fs.createWriteStream(part, { flags:offset ? 'a' : 'w', mode:0o600 });
    const discardPart = () => {
      const removeNow = () => {
        try { fs.rmSync(part, { force:true }); }
        catch (_) { try { fs.unlink(part, () => {}); } catch (_) {} }
      };
      // On Windows an open WriteStream keeps the staging file locked.  If the
      // descriptor is already closed, remove synchronously so an upload-cancel
      // response (and tests/cleanup immediately following it) cannot race the
      // asynchronous unlink.  Otherwise defer removal until close.
      if (ws.closed) removeNow();
      else ws.once('close', removeNow);
    };
    ...
    ws.on('close', () => {
      if (failed || !writeFinished) return;
      // Finalize only after the local writer has released its descriptor. This
      // avoids Windows EPERM races when an incomplete resumable upload is then
      // cancelled, retargeted, or cleaned up immediately by the caller.
      if (written < total) {
        if (uploadId) d.uploadsInFlight.delete(uploadId);
