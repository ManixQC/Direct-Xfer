...
function createPublicAccessService(deps = {}) {
  ...
  const unlockSecret = crypto.randomBytes(32);
  ...
  async function checkSharePassword(share, password) {
    ...
    // Legacy salted SHA-256 (shares created before the scrypt upgrade).
    const digest = crypto.createHash('sha256').update((share.pwSalt || '') + ':' + password).digest('hex');
    return { ok: true, match: timingSafeEqualStr(digest, share.pwHash) };
  }

  async function upgradeLegacySharePassword(share, password) {
    if (!share || !share.pwHash || parseHash(share.pwHash)) return false;
    const upgraded = await hashPassword(password);
    if (!upgraded.ok) return false;
    share.pwHash = upgraded.hash;
    delete share.pwSalt;
    return true;
  }
