...
test('1.69.3 launcher defers optional-component housekeeping until backend startup completes', () => {
...
test('1.69.3 ServerHost validates app runtime and Node concurrently', () => {
...
test('1.69.3 Node hashing uses a large sequential buffer and pinned private Node skips redundant version process', () => {
...
test('1.69.3 search cache hydration is deferred while transfer-log trimming stays race-safe before listen', () => {
...
test('1.69.3 internal Windows build identifiers reflect startup optimization revision', () => {
  assert.match(launcher, /RuntimeAppBuild = "1\.69\.3-launcher126-csharp"/);
  assert.match(launcher, /ServerHostBuild = "1\.69\.3-serverhost99-csharp"/);
  assert.match(host, /RuntimeAppBuild = "1\.69\.3-launcher126-csharp"/);
  assert.match(host, /HostVersion = "1\.69\.3-serverhost99-csharp"/);
});
