...
function parseTrustProxy(v) {
  ...
  // Express/proxy-addr accepts explicit IP/CIDR trust lists. Preserve only
  // literal IPv4/IPv6/CIDR tokens; named presets and arbitrary strings are
  // rejected so L3 deployments can bind intermediary trust to concrete peers.
  const tokens = raw.split(/[\s,;]+/).filter(Boolean);
  const validLiteral = (token) => {
  ...
  if (tokens.length && tokens.every(validLiteral)) return tokens.join(', ');
  return false;
}
