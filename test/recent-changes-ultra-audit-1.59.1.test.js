...
test('1.59.1 hardening remains present after the 1.63.4 ServerHost split', () => {
  assert.match(launcher, /AssemblyVersion\("1\.63\.4\.0"\)/);
  assert.match(host, /AssemblyVersion\("1\.63\.4\.0"\)/);
  assert.match(launcher, /RuntimeAppBuild = "1\.63\.4-launcher52-csharp"/);
  assert.match(host, /RuntimeAppBuild = "1\.63\.4-launcher52-csharp"/);
  assert.match(workflow, /DX_VERSION: '1\.63\.4'/);
  assert.match(iss, /#define AppVersion "1\.63\.4"/);
});

test('PWA cache generation is advanced consistently for 1.63.4', () => {
...
test('all ServerHost critical runtime hashes match actual normalized 1.63.4 files', () => {
...
test('installer upgrade and GitHub release gates remain hardened', () => {
  ...
  assert.match(workflow, /recent-changes-ultra-audit-1\.63\.4\.test\.js/);
