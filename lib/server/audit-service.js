...
function createAuditService(deps) {
  ...
  const externalCrypto = ASVS_L3_MODE ? createExternalCryptoProvider({ command:env.ASVS_L3_CRYPTO_COMMAND }) : null;
  if (ASVS_L3_MODE && !externalCrypto) throw Object.assign(new Error('asvs-l3-external-crypto-provider-required'), { code:'ASVS_L3_CRYPTO_PROVIDER_REQUIRED' });
  if (ASVS_L3_MODE && (AUDIT_HMAC_SECRET || AUDIT_SIGNING_PRIVATE_ENV || AUDIT_SIGNING_PRIVATE_PATH)) {
    throw Object.assign(new Error('asvs-l3-local-long-lived-audit-key-forbidden'), { code:'ASVS_L3_LOCAL_KEY_FORBIDDEN' });
  }
  const EXTERNAL_AUDIT_KEY = externalCrypto ? Object.freeze({ external:true, keyId:externalCrypto.keyId('audit-hmac') }) : null;
  const AUDIT_REMOTE_URL = String(env.AUDIT_REMOTE_URL || '').trim();
  const AUDIT_REMOTE_TOKEN = String(env.AUDIT_REMOTE_TOKEN || '').trim();
  const AUDIT_REMOTE_QUEUE_MAX = Math.max(100, Math.min(10000, Number(env.AUDIT_REMOTE_QUEUE_MAX) || 2000));
  const AUDIT_KEY_MODE = externalCrypto ? 'external-provider' : (AUDIT_HMAC_SECRET ? (AUDIT_HMAC_ENV_SECRET ? 'env' : 'data-key') : 'local-file');
  ...
  const remoteAuditQueue = [];
  let remoteAuditSending = false;
  let remoteAuditRetryTimer = null;
  let remoteAuditFailures = 0;
  let remoteAuditLastSuccessAt = 0;
  let remoteAuditLastError = null;
  const remoteAuditEndpoint = (() => {
    if (!AUDIT_REMOTE_URL) return null;
    try {
      const parsed = new URL(AUDIT_REMOTE_URL);
      if (!(parsed.protocol === 'https:' && parsed.hostname && !parsed.username && !parsed.password)) return null;
      assertAsvsL3OutboundUrl(parsed.toString(), { asvsL3Mode:ASVS_L3_MODE, allowlist:ASVS_L3_EGRESS_ALLOWLIST });
      return parsed;
    } catch (error) {
      if (ASVS_L3_MODE) throw error;
      return null;
    }
  })();

  function scheduleRemoteAuditDrain(delayMs = 0) {
    if (!remoteAuditEndpoint || remoteAuditRetryTimer || remoteAuditSending || !remoteAuditQueue.length) return;
    remoteAuditRetryTimer = setTimeout(() => {
      remoteAuditRetryTimer = null;
      drainRemoteAuditQueue();
    }, Math.max(0, delayMs));
    if (remoteAuditRetryTimer && typeof remoteAuditRetryTimer.unref === 'function') remoteAuditRetryTimer.unref();
  }

  function sendRemoteAuditEntry(entry) {
    return new Promise((resolve, reject) => {
      const body = Buffer.from(JSON.stringify({ source:APP_NAME, version:APP_VERSION, event:entry }) + '\n');
      const headers = {
        'Content-Type':'application/x-ndjson',
        'Content-Length':String(body.length),
        'User-Agent':`${APP_NAME}/${APP_VERSION} audit`,
        'X-Direct-Xfer-Audit-Hash':String(entry && entry.hash || ''),
      };
      if (AUDIT_REMOTE_TOKEN) headers.Authorization = `Bearer ${AUDIT_REMOTE_TOKEN}`;
      const req = https.request(remoteAuditEndpoint, {
        method:'POST', headers, timeout:10000, rejectUnauthorized:true,
      }, (res) => {
        res.resume();
        const status = Number(res.statusCode) || 0;
        if (status >= 200 && status < 300) resolve();
        else reject(new Error(`remote-audit-http-${status}`));
      });
      req.on('timeout', () => req.destroy(new Error('remote-audit-timeout')));
      req.on('error', reject);
      req.end(body);
    });
  }

  async function drainRemoteAuditQueue() {
    if (!remoteAuditEndpoint || remoteAuditSending || !remoteAuditQueue.length) return;
    remoteAuditSending = true;
    try {
      while (remoteAuditQueue.length) {
        try {
          await sendRemoteAuditEntry(remoteAuditQueue[0]);
          remoteAuditQueue.shift();
          remoteAuditFailures = 0;
          remoteAuditLastError = null;
          remoteAuditLastSuccessAt = Date.now();
        } catch (error) {
          remoteAuditFailures += 1;
          remoteAuditLastError = String(error && error.message || error).slice(0,160);
          break;
        }
      }
    } finally {
      remoteAuditSending = false;
      if (remoteAuditQueue.length && !remoteAuditRetryTimer) {
        const backoff = Math.min(60000, 1000 * 2 ** Math.min(6, Math.max(0, remoteAuditFailures - 1)));
        scheduleRemoteAuditDrain(backoff);
      }
    }
  }

  function queueRemoteAudit(entry) {
    if (!remoteAuditEndpoint || !entry) return;
    if (remoteAuditQueue.length >= AUDIT_REMOTE_QUEUE_MAX) {
      remoteAuditQueue.shift();
      remoteAuditLastError = 'remote-audit-queue-overflow';
    }
    remoteAuditQueue.push(JSON.parse(JSON.stringify(entry)));
    scheduleRemoteAuditDrain(0);
  }

  function getRemoteAuditStatus() {
    return {
      configured:!!remoteAuditEndpoint,
      secure:!!remoteAuditEndpoint,
      queued:remoteAuditQueue.length,
      sending:remoteAuditSending,
      failures:remoteAuditFailures,
      lastSuccessAt:remoteAuditLastSuccessAt || null,
      lastError:remoteAuditLastError,
    };
  }
...
function auditKeyId(key) {
  if (key && key.external === true) return String(key.keyId || 'external').slice(0, 64);
...
function ensureAuditChainKey() {
  ...
  if (externalCrypto) { auditChainKey = EXTERNAL_AUDIT_KEY; auditActiveKeyMode = 'external-provider'; return auditChainKey; }
...
function ensureAuditProofKeys() {
  if (externalCrypto) {
    if (!auditProofPublicKey) auditProofPublicKey = crypto.createPublicKey(externalCrypto.signingPublicKey);
    if (auditProofPublicKey.asymmetricKeyType !== 'ed25519') throw Object.assign(new Error('audit-signing-key-not-ed25519'), { code:'audit-signing-key-invalid' });
    return { external:true, privateKey:null, publicKey:auditProofPublicKey };
  }
...
function buildAuditProof(entries, integrity) {
  ...
  proof.signature = (keys.external ? externalCrypto.sign(Buffer.from(auditProofPayload(proof))) : crypto.sign(null, Buffer.from(auditProofPayload(proof)), keys.privateKey)).toString('base64');
  return proof;
}
...
const AUDIT_ENTRY_ALLOWED_FIELDS = new Set(['version','seq','at','action','actor','actorId','role','ip','detail','authMethod','authResult','prevHash','hash']);
function auditEntryShapeReason(entry) {
  ...
  const version = entry.version == null ? 1 : entry.version;
  if (version !== 1 && version !== 2) return 'entry-version-unsupported';
  // v1 is the historical Direct-Xfer journal schema. Authentication metadata is
  // only accepted in v2 so it can never be appended as mutable unsigned data to
  // an old HMAC payload.
  if (version === 1 && (Object.prototype.hasOwnProperty.call(entry, 'authMethod') || Object.prototype.hasOwnProperty.call(entry, 'authResult'))) return 'entry-extra-field';
  ...
  if (version === 2) {
    if (entry.authMethod != null && (typeof entry.authMethod !== 'string' || entry.authMethod.length > 64)) return 'entry-shape-invalid';
    if (entry.authResult != null && !['success','failure'].includes(entry.authResult)) return 'entry-shape-invalid';
  }
...
function canonicalAuditEntry(old, seq, prevHash) {
  const version = old && old.version === 2 ? 2 : 1;
  const entry = {
    seq,
  ...
  if (version === 2) {
    entry.version = 2;
    entry.authMethod = old && old.authMethod != null ? String(old.authMethod).slice(0, 64) : null;
    entry.authResult = old && ['success','failure'].includes(old.authResult) ? old.authResult : null;
  }
  return entry;
}
...
function auditChainPayload(entry) {
  ...
  // Preserve the exact historical v1 payload for existing journals. New v2
  // entries sign normalized authentication method/result as first-class audit
  // metadata so those fields are tamper-evident as well.
...
function auditChainHashWithKey(entry, key) {
  if (key && key.external === true) return externalCrypto.hmac(auditChainPayload(entry));
...
function auditHeadSealWithKey(seq, hash, key) {
  const payload = 'head|' + String(seq) + '|' + String(hash || '');
  if (key && key.external === true) return externalCrypto.hmac(payload);
...
function initAuditChain() {
  ...
  if (status.ok) {
    ...
    if (ASVS_L3_MODE) throw Object.assign(new Error('asvs-l3-audit-integrity-required:' + (status.reason || 'verification-failed')), { code:'ASVS_L3_AUDIT_INTEGRITY' });
  }
}
...
function inferAuthenticationMethod(action, opts = {}) {
  if (opts.authMethod) return String(opts.authMethod).slice(0, 64);
  const name = String(action || '').toLowerCase();
  if (name.includes('passkey') || name.includes('webauthn')) return 'passkey';
  if (name.includes('totp') || name.includes('2fa')) return 'totp-or-recovery';
  if (name.startsWith('login')) return 'password';
  return null;
}

function authenticationResult(action) {
  const name = String(action || '').toLowerCase();
  if (!/(login|passkey|webauthn|totp|2fa|authenticat)/.test(name)) return null;
  if (/(fail|denied|invalid|locked|required|error|reject)/.test(name)) return 'failure';
  return 'success';
}
...
function logAudit(action, opts) {
  ...
  const entry = {
    ...
    ip: opts.ip || null,
    detail: detailValue,
    authMethod: inferAuthenticationMethod(action, opts),
    authResult: authenticationResult(action),
  };
