...
test('Windows metadata targets 1.66.6 and current runtime hashes', () => {
  const launcher = read('windows-launcher/Program.cs');
  const host = read('windows-server-host/Program.cs');
  assert.match(launcher, /AppVersion = "1\.66\.6"/);
  assert.match(launcher, /RuntimeAppBuild = "1\.66\.6-launcher82-csharp"/);
  assert.match(host, /RuntimeAppBuild = "1\.66\.6-launcher82-csharp"/);
  assert.match(host, /HostVersion = "1\.66\.6-serverhost55-csharp"/);
...
test('Windows GitHub Actions run name follows Direct-Xfer 1.66.6', () => {
  const workflow = read('.github/workflows/build-windows-csharp.yml');
