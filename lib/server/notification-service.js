...
const { assertAsvsL3OutboundUrl, assertAsvsL3OutboundHost, sanitizeMailHeader } = require('./asvs-l3-policy');
...
function createNotificationService(deps) {
  ...
  function sendWebhook(url, format, message, kind, payload) {
    ...
    try {
      assertAsvsL3OutboundUrl(url, { enabled:ASVS_L3_MODE === true, allowlist:ASVS_L3_EGRESS_ALLOWLIST });
      return fetch(url, { method: 'POST', headers: { 'Content-Type': contentType }, body, signal: AbortSignal.timeout(5000), redirect:'error', cache:'no-store' })
  ...
  function getMailer() {
    ...
    if (ASVS_L3_MODE === true) {
      if (e.url) {
        let parsed;
        try { parsed = new URL(e.url); } catch (_) { throw Object.assign(new Error('invalid-smtp-url'), { code:'invalid-smtp-url' }); }
        if (!['smtp:', 'smtps:'].includes(parsed.protocol) || !parsed.hostname) throw Object.assign(new Error('invalid-smtp-url'), { code:'invalid-smtp-url' });
        const port = parsed.port || (parsed.protocol === 'smtps:' ? '465' : '25');
        assertAsvsL3OutboundHost(parsed.hostname, port, { enabled:true, allowlist:ASVS_L3_EGRESS_ALLOWLIST, protocol:parsed.protocol });
      } else {
        assertAsvsL3OutboundHost(e.host, String(e.port || ''), { enabled:true, allowlist:ASVS_L3_EGRESS_ALLOWLIST, protocol:e.secure ? 'smtps:' : 'smtp:' });
      }
    }
    const key = JSON.stringify([e.url || '', e.host || '', e.port || '', e.secure || false, e.user || '', e.pass || '', ASVS_L3_MODE === true]);
    if (mailerCache && mailerCache.key === key) return mailerCache.transport;
    const transport = e.url
      ? nodemailer.createTransport(e.url, ASVS_L3_MODE === true ? { requireTLS:true } : undefined)
      : nodemailer.createTransport({
          host: e.host, port: e.port, secure: e.secure,
          requireTLS:ASVS_L3_MODE === true,
  ...
  async function sendMail(subject, text, toOverride) {
    ...
    let to, from, safeSubject;
    try {
      to = sanitizeMailHeader((toOverride && String(toOverride).trim()) || e.to, 400);
      from = sanitizeMailHeader(e.from, 200);
      safeSubject = sanitizeMailHeader(subject, 300);
    } catch (err) {
      const message = String(err && (err.code || err.message) || 'unsafe-mail-header').slice(0, 240);
      lastEmail = { at: Date.now(), ok: false, error: message };
      return lastEmail;
    }
    if (!tx || !to || !from) { lastEmail = { at: Date.now(), ok: false, error: 'not-configured' }; return lastEmail; }
    try {
      await tx.sendMail({ from, to, subject:safeSubject, text:String(text == null ? '' : text).slice(0, 2 * 1024 * 1024) });
  ...
  // Push delivery can mutate persisted subscription state when a vendor reports
  // an expired endpoint. Keep those asynchronous operations visible to the restore
  // coordinator so a backup restore cannot replace the root state while an older
  // delivery is still capable of pruning subscriptions from the new state.
  const stateReplacementJobs = new Set();
  function trackDetachedStateReplacementJob(promise) {
    const job = Promise.resolve(promise);
    stateReplacementJobs.add(job);
    job.then(
      () => { stateReplacementJobs.delete(job); },
      () => { stateReplacementJobs.delete(job); },
    );
    return job;
  }
  function isBusyForStateReplacement() { return stateReplacementJobs.size > 0; }
  ...
  function sendWebPush(kind, title, body, payload, subs) {
    ...
    for (const sub of targets) {
      try {
        const delivery = Promise.resolve(webpush.sendNotification({ endpoint: sub.endpoint, keys: sub.keys }, data, opts))
          .then(() => { try { noteCenterServiceState('web-push-delivery', true, 'Service Push rétabli'); } catch (_) {} })
          .catch((err) => handlePushError(sub, err));
        trackDetachedStateReplacementJob(delivery);
      } catch (err) { handlePushError(sub, err); }
    }
  ...
  async function sendWebPushAwaited(kind, title, body, payload, sub) {
    ...
    // Keep the barrier until *after* rejection handling has pruned any stale
    // subscription. Tracking only the vendor promise would open a microtask-sized
    // window where restore could observe idle state before the catch mutates meta.
    const stateJob = Object.freeze({ kind:'web-push-awaited', sentAt });
    stateReplacementJobs.add(stateJob);
    try {
      try {
      ...
      stateReplacementJobs.delete(stateJob);
    }
  }
  ...
  return {
    ...
    getLastWebhook, getLastEmail, isBusyForStateReplacement, clearRuntimeState,
  };
}
