...
test('1.60.1 release metadata is synchronized across Node, PWA, launcher, host and installer', () => {
  const pkg = JSON.parse(read('package.json'));
  const lock = JSON.parse(read('package-lock.json'));
  assert.equal(pkg.version, '1.60.1');
  assert.equal(lock.version, '1.60.1');
  assert.equal(lock.packages[''].version, '1.60.1');
  assert.match(read('pwa', 'app.js'), /APP_VERSION = '1\.60\.1'/);
  assert.match(read('pwa', 'app.js'), /APP_BUILD = '2026\.08\.15-pwa290'/);
  assert.match(read('pwa', 'sw.js'), /VERSION = '2026\.08\.15-pwa290'/);
  assert.match(read('pwa', 'index.html'), /v1\.60\.1 · pwa290/);
  assert.match(launcher, /AppVersion = "1\.60\.1"/);
  assert.match(launcher, /RuntimeAppBuild = "1\.60\.1-launcher36-csharp"/);
  assert.match(host, /HostVersion = "1\.60\.1-serverhost9-csharp"/);
  assert.match(workflow, /DX_VERSION: '1\.60\.1'/);
  assert.match(workflow, /DX_RUNTIME_BUILD: '1\.60\.1-launcher36-csharp'/);
  assert.match(workflow, /DX_SERVER_HOST_BUILD: '1\.60\.1-serverhost9-csharp'/);
  assert.match(iss, /#define AppVersion "1\.60\.1"/);
});
...
test('PWA and admin notification assets are cache-busted consistently while the favicon remains v269', () => {
  ...
  assert.match(read('pwa', 'index.html'), /app\.js\?v=274/);
  assert.match(read('pwa', 'sw.js'), /app\.css\?v=272/);
  assert.match(read('pwa', 'sw.js'), /app\.js\?v=274/);
  assert.match(read('public', 'index.html'), /style\.css\?v=278/);
  assert.match(read('public', 'index.html'), /app\.js\?v=277/);
