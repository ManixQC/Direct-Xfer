...
Release: Direct-Xfer `1.71.24`
Baseline input: Direct-Xfer `1.70.26` ASVS-L3 release
Detailed matrix: `security/ASVS-5.0.0-L3-MATRIX.md`

## Release verification result

Direct-Xfer 1.71.24 is a focused CodeQL correction over 1.71.23. GitHub alert #17361 showed that even the HMAC callback binding introduced in 1.71.23 was still sensitive material persisted through `Set-Cookie`. The broker now persists no OAuth callback credential in browser cookies at all: browser ownership is checked before redirect with the high-entropy launch binding, while the provider callback is correlated by one-time `state` and bound to the server-side session by PKCE. The HIBP protocol exception and PWA Stylelint fixes from 1.71.23 remain intact. The ASVS status matrix remains 253 PASS / 92 N/A / 0 MANUAL / 0 PARTIAL / 0 FAIL / 0 REVIEW.

### 1.71.24 Code Scanning hardening
...
- Application/package metadata is synchronized to `1.71.24` across the root package/lockfile, OAuth broker packages, Windows workflow/installer metadata, SBOM, OpenVEX, ASVS release evidence and release-scoped regression fixtures.
- PWA generation advances to `pwa487` with cache-buster `v=468` so clients cannot retain stale release metadata from the previous shell cache.
...
- Security inventory regenerated for 1.71.24 — **957 entries**.
...
- CycloneDX SBOM root component synchronized to 1.71.24.
...
- [x] package and lockfile version synchronized to 1.71.24.
- [x] PWA version/cache generation synchronized to 1.71.24 / pwa487.
...
- [x] SBOM root component synchronized to 1.71.24.
