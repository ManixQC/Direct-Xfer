...
function attachAdminSecurityRoutes(deps = {}) {
  ...
  if (!sessionService || !authService) throw new TypeError('admin-security-routes requires sessionService/authService');
  ...
  const verifyCurrentPassword = authService.verifyCurrentPassword;
  if (typeof verifyCurrentPassword !== 'function') throw new TypeError('admin-security-routes requires authService.verifyCurrentPassword()');
  ...
  adminRouter.get('/security/overview', (req, res) => {
    const currentSid = req.session && req.session.sid;
    const activeSessions = [];
    const now = Date.now();
    const canViewAll = ['owner', 'admin', 'auditor'].includes(req.session.role);
    for (const session of sessionService.listSessions(now)) {
      if (!canViewAll && String(session.accountId || '') !== String(req.session.accountId || '')) continue;
      const ownSession = String(session.accountId || '') === String(req.session.accountId || '');
      activeSessions.push({
        ...
        canRevoke: ownSession || ['owner', 'admin'].includes(req.session.role),
      });
    }
  ...
  adminRouter.delete('/security/sessions/:id', async (req, res) => {
    const wanted = String(req.params.id || '');
    let found = null;
    for (const session of sessionService.listSessions()) {
      if (sessionPublicHandle(session.sid) === wanted) { found = session; break; }
    }
    if (!found) return res.status(404).json({ error: 'not-found' });
    const ownSession = String(found.accountId || '') === String(req.session.accountId || '');
    if (!ownSession && !['owner', 'admin'].includes(req.session.role)) return res.status(403).json({ error:'forbidden' });

    // ASVS V7.5.2: session termination is step-up protected. Reauthenticate the
    // actor immediately before revocation rather than trusting a potentially old
    // dashboard session.
    const actor = getAccountById(req.session.accountId);
    if (!actor) return res.status(401).json({ error:'not-authenticated' });
    const currentCheck = await verifyCurrentPassword(actor, String(req.body && req.body.password || ''));
    if (!currentCheck.ok) return res.status(503).json({ error:currentCheck.error || 'auth-busy' });
    if (!currentCheck.match) return res.status(403).json({ error:'reauth-failed' });
    if (!sessionService.isSessionActive(req.session.sid, [req.session.role])) return res.status(401).json({ error:'not-authenticated' });
    ...
    if (current) {
      const suffix = secureCookie(req);
      res.setHeader('Set-Cookie', [
        `sid=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0${suffix}`,
        `__Host-sid=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0${suffix}`,
      ]);
    }
