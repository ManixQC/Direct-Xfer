...
test('1.69.2 api helper extraction is portable across LF and CRLF checkouts', () => {
...
test('1.69.2 api GET/HEAD never attach a request body even when null is passed as timeout placeholder', async () => {
  const calls = [];
  const context = {
...
test('1.69.2 connector and web-storage reads still use the shared api helper', () => {
...
test('1.69.2 write requests still serialize JSON and include CSRF', async () => {
  const calls = [];
  const context = {
    state:{ authEpoch:1, csrf:'csrf-token' },
    showLogin(){},
