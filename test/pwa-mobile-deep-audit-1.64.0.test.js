...
test('1.71.8 release and pwa471 are synchronized',()=>{
  assert.match(read('package.json'),/"version"\s*:\s*"1\.71\.8"/);
  for(const f of ['pwa/app.js','pwa/index.html','pwa/sw.js','pwa/theme-init.js','pwa/admin-advanced.js','pwa/mobile-intelligence.js']) assert.match(read(f),/1\.71\.8|pwa471|v=452/);
...
test('shortcut manifests use the current pwa471 cache buster on app and login paths',()=>{
  assert.match(app,/manifestHref = '\/direct-xfer-pwa\.webmanifest\?v=452'/);
  assert.match(read('pwa/login.html'),/direct-xfer-pwa\.webmanifest\?v=452/);
});
