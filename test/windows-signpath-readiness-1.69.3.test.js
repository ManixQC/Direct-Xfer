...
test('1.69.6 one-shot Windows install choices are persisted then remain editable', () => {
  const host = read('windows-server-host/Program.cs');
  const bootstrap = read('lib/server/state-bootstrap-service.js');
  ...
  assert.match(bootstrap, /applyWindowsInstallPreferences\(currentState, env \|\| \{\}\)/);
...
test('1.69.6 public-IP discovery can be disabled before any public-IP fetch', async () => {
  ...
  try {
    ...
    const services = createNetworkServices({
      net: require('node:net'), os: require('node:os'), LOCAL_IP:'', APP_VERSION:'1.69.6', UPDATE_REPO:'owner/repo', UPDATE_TAG:'latest',
...
test('1.69.6 release template and repository policy meet SignPath page requirements', () => {
