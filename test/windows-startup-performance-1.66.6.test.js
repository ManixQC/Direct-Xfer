...
test('1.67.6 launcher defers optional-component housekeeping until backend startup completes', () => {
...
test('1.67.6 ServerHost validates app runtime and Node concurrently', () => {
...
test('1.67.6 Node hashing uses a large sequential buffer and pinned private Node skips redundant version process', () => {
...
test('1.67.6 search cache hydration is deferred while transfer-log trimming stays race-safe before listen', () => {
...
test('1.67.6 internal Windows build identifiers reflect startup optimization revision', () => {
  assert.match(launcher, /RuntimeAppBuild = "1\.67\.6-launcher93-csharp"/);
  assert.match(launcher, /ServerHostBuild = "1\.67\.6-serverhost66-csharp"/);
  assert.match(host, /RuntimeAppBuild = "1\.67\.6-launcher93-csharp"/);
  assert.match(host, /HostVersion = "1\.67\.6-serverhost66-csharp"/);
});
