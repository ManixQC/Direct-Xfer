...
test('1.68.3 installed UI offers broker deployment without source files or Wrangler', () => {
...
test('1.68.3 embedded Cloudflare assets stay byte-identical to the standalone broker', () => {
...
test('1.68.3 automatic deployment uses Cloudflare APIs directly and never shells out to Wrangler', () => {
...
test('1.68.3 Cloudflare token is kept server-side during the wizard and never returned to the browser', async () => {
...
test('1.68.3 reports the exact Cloudflare deployment stage instead of a generic failure', async () => {
...
test('1.68.3 validates Cloudflare token status before changing account resources', async () => {
  const originalFetch = global.fetch;
  let calls = 0;
  global.fetch = async (url) => {
    calls += 1;
