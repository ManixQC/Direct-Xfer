...
test('Windows metadata targets 1.64.10 and current runtime hashes', () => {
  const launcher = read('windows-launcher/Program.cs');
  const host = read('windows-server-host/Program.cs');
  assert.match(launcher, /AppVersion = "1\.64\.10"/);
  assert.match(launcher, /RuntimeAppBuild = "1\.64\.10-launcher60-csharp"/);
  assert.match(host, /HostVersion = "1\.64\.10-serverhost33-csharp"/);
