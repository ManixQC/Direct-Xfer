...
function createOAuthBrokerDeploymentRoutes(deps) {
  const { adminRouter, requireFullAdmin, crypto, auditReq, ASVS_L3_MODE = false, ASVS_L3_EGRESS_ALLOWLIST = '' } = deps;
  ...
  async function cfFetch(token, pathname, options = {}) {
    ...
    try {
      ...
      const targetUrl = `${CF_API}${pathname}`;
      assertAsvsL3OutboundUrl(targetUrl, { asvsL3Mode:ASVS_L3_MODE, allowlist:ASVS_L3_EGRESS_ALLOWLIST });
      const response = await fetch(targetUrl, { method:options.method || 'GET', headers, body, signal:controller.signal, redirect:ASVS_L3_MODE ? 'error' : 'follow' });
      let data = null;
  ...
  async function brokerInfo(url, attempts = 10) {
    ...
    for (let attempt = 0; attempt < attempts; attempt += 1) {
      ...
      try {
        const targetUrl = `${url}/v1/info`;
        assertAsvsL3OutboundUrl(targetUrl, { asvsL3Mode:ASVS_L3_MODE, allowlist:ASVS_L3_EGRESS_ALLOWLIST });
        const response = await fetch(targetUrl, { headers:{ accept:'application/json', 'cache-control':'no-cache' }, signal:controller.signal, redirect:ASVS_L3_MODE ? 'error' : 'follow' });
