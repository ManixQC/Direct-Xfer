...
test('1.71.15 builds Tesseract 5.5.3 from the exact signed-release commit with optional surfaces disabled', () => {
...
test('1.71.15 final image omits Debian Tesseract binaries and keeps only required OCR language/runtime packages', () => {
  ...
  assert.match(runtime, /TESSDATA_PREFIX=\/usr\/share\/tesseract-ocr\/5 tesseract --list-langs/);
  assert.doesNotMatch(runtime, /TESSDATA_PREFIX=\/usr\/share\/tesseract-ocr\/5\/tessdata/);
  ...
  assert.match(dockerfile, /TESSDATA_PREFIX=\/usr\/share\/tesseract-ocr\/5 \\/);
  assert.doesNotMatch(dockerfile, /TESSDATA_PREFIX=\/usr\/share\/tesseract-ocr\/5\/tessdata/);
  assert.ok((runtime.match(/RUN set -eux;/g) || []).length >= 4, 'runtime setup must stay split into diagnostic layers');
});

test('1.71.15 OpenVEX is narrow, package-scoped, and removes the old unsound Cairo suppression', () => {
  assert.equal(openvex['@id'], 'https://github.com/ManixQC/Direct-Xfer/security/openvex-1.71.15');
...
test('1.71.15 keeps the intentional Docker root bootstrap exception narrow and reviewable', () => {
...
test('1.71.15 Trivy produces a complete report but gates only fixable, VEX-reviewed findings', () => {
...
test('1.71.15 OAuth broker image runs as the unprivileged node user with writable persistent data', () => {
