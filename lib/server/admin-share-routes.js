...
adminRouter.get('/shares', async (req, res) => {
  ...
  adminRouter.delete('/trash/:id',requireFullAdmin,async(req,res)=>{try{const rec=await purgeTrashRecordById(req.params.id,req);if(!rec)return res.status(404).json({error:'not-found'});scheduleSearchReindex();auditReq(req,'trash-purged',rec.share?rec.share.name:req.params.id);res.json({ok:true,persisted:true});}catch(e){console.error('[trash] purge failed:',e&&e.message);res.status(e&&e.code==='write-error'?503:500).json({error:e&&e.code==='write-error'?'write-error':'delete-failed',persisted:e&&e.code==='write-error'?false:undefined});}});
  adminRouter.delete('/trash',requireFullAdmin,async(req,res)=>{const ids=trashItems().filter((r)=>trashRecordVisible(req,r)).map((r)=>r.id);let count=0,failed=0,writeFailed=0;for(const id of ids){try{if(await purgeTrashRecordById(id,req))count++;}catch(e){failed++;if(e&&e.code==='write-error')writeFailed++;console.error('[trash] purge failed:',id,e&&e.message);}}if(count){scheduleSearchReindex();auditReq(req,'trash-purged-all',`${count}; failed=${failed}`);}if(writeFailed)return res.status(503).json({ok:false,error:'write-error',persisted:false,count,failed});res.status(failed?207:200).json({ok:failed===0,count,failed,persisted:true});});
