...
test('1.71.27 rejects observations older than seven days even when the bundle itself is current', (t) => {
  ...
  const result=verifyEvidenceBundle(stale,{appVersion:'1.71.27',publicUrl:'https://direct-xfer.example',publicKey:ev.keyPair.publicKey,now});
  assert.equal(result.ok,false); assert.ok(result.failures.some((row)=>row.id===stale.checks[0].id));
});

test('1.71.27 L3 state loader rejects plaintext and legacy dxenc:1 stores', (t) => {
...
test('1.71.27 L3 backups are encrypted even though DATA_KEY is absent', () => {
...
test('1.71.27 purges plaintext search and OCR caches on L3 startup', (t) => {
...
test('1.71.27 proxy trust accepts only valid non-global IP/CIDR literals', () => {
...
test('1.71.27 refuses a symlinked external crypto command', (t) => {
...
test('1.71.27 migrates the legacy audit chain to the isolated provider and updates the encrypted state anchor', (t) => {
...
test('1.71.27 converts a 1.70.26 plaintext L3 backup to dxenc:2 offline', (t) => {
...
test('1.71.27 accepts packed hardware attestation when x5c omits the pinned root certificate', (t) => {
