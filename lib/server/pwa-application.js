...
const PWA_ROUTE_FACADE_CONTEXT = Object.freeze({
  runtime: Object.freeze({
    ...
    sanitizeImageMetadataFile:['photo-utils', 'sanitizeImageMetadataFile'],
  ...
  identity: Object.freeze({
    ...
    clearOtherSessionsOfAccount:['session', 'clearOtherSessionsOfAccount'],
...
function createPwaApplication(options = {}) {
  ...
  const pwaAsvsL3Mode = contextValue(context, 'config', 'ASVS_L3_MODE') === true;
  function optionalConfigValue(name) {
    const source = contextDomain(context, 'config');
    const descriptor = Object.getOwnPropertyDescriptor(source, name);
    return descriptor && Object.prototype.hasOwnProperty.call(descriptor, 'value') && descriptor.value !== undefined ? descriptor.value : '';
  }

  const webauthnService = createWebauthnService({
    APP_NAME: contextValue(context, 'config', 'APP_NAME'),
    PUBLIC_URL: contextValue(context, 'config', 'PUBLIC_URL'),
    ASVS_L3_MODE: pwaAsvsL3Mode,
    ASVS_L3_HARDWARE_AAGUIDS: pwaAsvsL3Mode ? contextValue(context, 'config', 'ASVS_L3_HARDWARE_AAGUIDS') : optionalConfigValue('ASVS_L3_HARDWARE_AAGUIDS'),
    ASVS_L3_ATTESTATION_ROOT_SHA256: pwaAsvsL3Mode ? contextValue(context, 'config', 'ASVS_L3_ATTESTATION_ROOT_SHA256') : optionalConfigValue('ASVS_L3_ATTESTATION_ROOT_SHA256'),
    ASVS_L3_ATTESTATION_ROOT_FILES: pwaAsvsL3Mode ? contextValue(context, 'config', 'ASVS_L3_ATTESTATION_ROOT_FILES') : optionalConfigValue('ASVS_L3_ATTESTATION_ROOT_FILES'),
    ASVS_L3_CRYPTO_COMMAND: pwaAsvsL3Mode ? contextValue(context, 'config', 'ASVS_L3_CRYPTO_COMMAND') : optionalConfigValue('ASVS_L3_CRYPTO_COMMAND'),
    crypto,
    getSession: contextValue(context, 'session', 'getSession'),
    getAccountById: contextValue(context, 'account', 'getAccountById'),
