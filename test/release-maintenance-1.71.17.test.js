...
test('1.71.17 keeps the audited dependency maintenance baseline', () => {
  assert.equal(pkg.version, '1.71.17');
...
test('1.71.17 PWA release metadata advances atomically', () => {
  for (const rel of ['pwa/app.js', 'pwa/sw.js', 'pwa/theme-init.js', 'pwa/index.html']) {
    const source = read(rel);
    assert.match(source, /1\.71\.17|pwa480|v=461/);
    assert.doesNotMatch(source, /1\.71\.12|pwa475|v=456/);
  }
  assert.match(read('pwa/app.js'), /APP_BUILD = '2026\.08\.26-pwa480'/);
  assert.match(read('pwa/sw.js'), /VERSION = '2026\.08\.26-pwa480'/);
});

test('1.71.17 Tesseract source acquisition targets the exact annotated release tag ref', () => {
