...
function pageShell(lang, title, bodyHtml) {
  ...
  const nonceBody = nonceAttr
    ? String(bodyHtml || '')
        .replace(/<script(?=[\s>])/g, `<script${nonceAttr}`)
        .replace(/<style(?=[\s>])/g, `<style${nonceAttr}`)
    : String(bodyHtml || '');
...
// Inline event handlers are intentionally forbidden by script-src-attr. Wire
// media fallbacks from this nonce-authorized block instead.
(function () {
  var videos = document.querySelectorAll('video[data-dx-video-fallback]');
  Array.prototype.forEach.call(videos, function (video) {
    video.addEventListener('error', function () {
      video.classList.add('is-hidden');
      var fallback = video.parentNode && video.parentNode.querySelector('.vfallback');
...
function filePage(lang, share, downloadUrl, tk, wm) {
  ...
  if (info && info.kind === 'image') {
    ...
    preview = `<div class="preview"><video src="${esc(viewUrl)}" data-dx-resume="1" data-dx-video-fallback="1" controls preload="metadata" playsinline></video>${wmHtml}<p class="vfallback muted is-hidden">${esc(L.vidUnsupported)} <a href="${esc(downloadUrl)}" download>${esc(L.download)}</a></p></div>`;
