...
test('1.71.14 builds Tesseract 5.5.3 from the exact signed-release commit with optional surfaces disabled', () => {
...
test('1.71.14 final image omits Debian Tesseract binaries and keeps only required OCR language/runtime packages', () => {
...
test('1.71.14 OpenVEX is narrow, package-scoped, and removes the old unsound Cairo suppression', () => {
  assert.equal(openvex['@id'], 'https://github.com/ManixQC/Direct-Xfer/security/openvex-1.71.14');
...
test('1.71.14 keeps the intentional Docker root bootstrap exception narrow and reviewable', () => {
...
test('1.71.14 Trivy produces a complete report but gates only fixable, VEX-reviewed findings', () => {
...
test('1.71.14 OAuth broker image runs as the unprivileged node user with writable persistent data', () => {
