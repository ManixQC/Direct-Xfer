...
const ROUTE_DOMAINS = Object.freeze({
  ...
  security:Object.freeze([
    'platform', 'core-utils', 'state-store', 'settings', 'account', 'audit', 'dlp', 'maintenance',
...
function createAdminApplication(options = {}) {
  ...
  const auditReq = requireFunction(audit.auditReq, 'audit.auditReq');

  const adminBoundary = createAdminRouter({ express, requireAuth, getAccountById, accountNeedsPwChange, getById, auditReq, asvsL3Mode:config.ASVS_L3_MODE === true, hasRecentStrongAuthentication:session.hasRecentStrongAuthentication });
