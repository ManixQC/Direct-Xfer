 ...
 * explicit runtime reset callbacks so replacing the root object never leaves a
...
function createRestoreService(deps = {}) {
  const {
    ...
    isBackupInFlight, getActiveTransferCount, hasActiveUploads,
    isShareStateReplacementBusy, isMaintenanceStateReplacementBusy,
    isConnectorJobStateReplacementBusy,
    clearAllSessions, clearTransferRuntimeState, clearDownloadRuntimeState,
    clearUploadRuntimeAfterRestore, clearPwaPairTickets,
    clearWebauthnRuntimeState, clearNotificationRuntimeState,
    clearPwaEventRuntimeState, closeActivityPresenceStreams, closeLiveActivityClients,
    clearNotificationCenterRuntimeState, clearPwaNotificationRuntimeState,
    clearMaintenanceRuntimeState, clearConnectorJobRuntimeState,
    clearSystemHealthRuntimeState, clearAccountBootstrapRuntime, resetSearchAfterRestore,
  ...
  function restoreIsBusy() {
    return !!(isBackupInFlight()
      || getActiveTransferCount() > 0
      || hasActiveUploads()
      || isShareStateReplacementBusy()
      || (typeof isMaintenanceStateReplacementBusy === 'function' && isMaintenanceStateReplacementBusy())
      || (typeof isConnectorJobStateReplacementBusy === 'function' && isConnectorJobStateReplacementBusy()));
  }
  ...
  function clearRuntimeAfterRestore() {
    ...
    reset('sessions', clearAllSessions);
    reset('transfers', clearTransferRuntimeState);
    reset('downloads', clearDownloadRuntimeState);
    reset('uploads', clearUploadRuntimeAfterRestore);
    reset('pwa-pair-tickets', clearPwaPairTickets);
    reset('webauthn', clearWebauthnRuntimeState);
    reset('notifications', clearNotificationRuntimeState);
    reset('pwa-events', clearPwaEventRuntimeState);
    reset('activity-presence', closeActivityPresenceStreams || closeLiveActivityClients);
    reset('notification-center', clearNotificationCenterRuntimeState);
    reset('pwa-notifications', clearPwaNotificationRuntimeState);
    reset('maintenance', clearMaintenanceRuntimeState);
    reset('connector-jobs', clearConnectorJobRuntimeState);
    reset('system-health', clearSystemHealthRuntimeState);
    reset('account-bootstrap', clearAccountBootstrapRuntime);
    reset('search', () => resetSearchAfterRestore(1000));
    if (failures.length) {
