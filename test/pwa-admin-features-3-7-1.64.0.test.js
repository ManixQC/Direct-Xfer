...
test('PWA cache build is synchronized to pwa405', () => {
  assert.match(theme, /2026\.08\.20-pwa405/);
  assert.match(theme, /admin-advanced\.js\?v=395/);
  assert.match(sw, /2026\.08\.20-pwa405/);
  assert.match(sw, /admin-advanced\.js\?v=395/);
  assert.match(read('pwa/app.js'), /2026\.08\.20-pwa405/);
  assert.match(read('pwa/login.js'), /v=395/);
  assert.match(read('pwa/index.html'), /pwa405/);
...
test('advanced-admin script is a public static PWA asset so first service-worker install can cache it', () => {
  ...
  assert.match(sw, /'\/app\/admin-advanced\.js\?v=395'/);
});
...
test('real server keeps new endpoints protected, serves PWA asset pre-login, and panic-pauses a scheduled share', { timeout:30000 }, async () => {
  ...
  try {
    ...
    const publicAsset = await fetch(base + '/app/admin-advanced.js?v=395');
    assert.equal(publicAsset.status, 200);
    assert.match(await publicAsset.text(), /pwa405/);
