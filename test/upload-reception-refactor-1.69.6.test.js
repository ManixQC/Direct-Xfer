...
test('1.69.6 upload/reception architecture is split behind explicit boundaries', () => {
  ...
  assert.match(runtimeServices, /createUploadReceptionService/);
  const publication = read('lib/server/application-publication.js');
  assert.match(publication, /attachReceptionCollaborationRoutes/);
  assert.match(runtimeServices, /const uploadReceptionService = createUploadReceptionService/);
...
test('1.69.6 upload service keeps mutable restore-sensitive state live and hides maintenance counters', () => {
  ...
  assert.match(runtimeServices, /live:\{[\s\S]*get state\(\) \{ return getState\(\); \}/);
  assert.match(service, /function maybeCleanupOrphanPendingFiles/);
  assert.match(runtimeServices, /runExpiredLinkLifecycle:method\(shareService,[\s\S]*maybeCleanupOrphanPendingFiles:method\(uploadReceptionService/);
  assert.match(maintenanceService, /maybeCleanupOrphanPendingFiles\(now\)/);
  assert.match(service, /function hasActiveUploads/);
  assert.match(stateLifecycle, /\['backup', \(\) => callLate\(runtimeProvider, 'runtimeServicesApplication', 'isBackupInFlight'\)\]/);
  assert.match(stateLifecycle, /\['uploads', \(\) => callLate\(runtimeProvider, 'runtimeServicesApplication', 'hasActiveUploads'\)\]/);
  assert.match(restoreService, /stateReplacementCoordinator\.isBusyForStateReplacement\(\)/);
  assert.match(service, /function clearRuntimeAfterRestore/);
  assert.match(stateLifecycle, /\['uploads', \(\) => callLate\(runtimeProvider, 'runtimeServicesApplication', 'clearUploadRuntimeAfterRestore'\)\]/);
  assert.match(server, /bootstrapReferences\.bindRuntime\(runtimeServicesApplication\)/);
  assert.match(restoreService, /stateReplacementCoordinator\.clearRuntimeAfterRestore\(\)/);
});
