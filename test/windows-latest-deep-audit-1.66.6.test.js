...
test('1.70.23 version, Windows builds and PWA generation are synchronized', () => {
  assert.equal(pkg.version, '1.70.23');
  assert.equal(lock.version, '1.70.23');
  assert.equal(lock.packages[''].version, '1.70.23');
  assert.match(workflow, /^run-name: v1\.70\.23$/m);
  ...
  for (const rel of ['pwa/theme-init.js','pwa/mobile-intelligence.js','pwa/index.html','pwa/admin-advanced.js','pwa/app.js','pwa/sw.js']) {
    assert.match(read(rel), /1\.70\.23|pwa456/);
  }
