...
test('1.71.12 keeps Express on the audited 4.x line and accepts Nodemailer 9.0.5', () => {
  assert.equal(pkg.version, '1.71.12');
...
test('1.71.12 Dependabot keeps Express major migration out of routine updates', () => {
...
test('1.71.12 adopts the validated GitHub Actions maintenance updates', () => {
...
test('1.71.12 PWA release metadata advances atomically', () => {
  for (const rel of ['pwa/app.js', 'pwa/sw.js', 'pwa/theme-init.js', 'pwa/index.html']) {
    const source = read(rel);
    assert.match(source, /1\.71\.12|pwa475|v=456/);
    assert.doesNotMatch(source, /1\.71\.9|pwa472|v=453/);
  }
  assert.match(read('pwa/app.js'), /APP_BUILD = '2026\.08\.26-pwa475'/);
  assert.match(read('pwa/sw.js'), /VERSION = '2026\.08\.26-pwa475'/);
});
