...
test('device-only status lists all devices paired to its account', async()=>{
  ...
  for (const current of [phone, tablet]) {
    ...
    const statusUrl=`${base}/app/device/status?version=${encodeURIComponent('1.59.1')}&build=${encodeURIComponent('2026.08.14-pwa280')}`;
    ...
    assert.equal(currentDevice.appVersion,'1.59.1');
    assert.equal(currentDevice.appBuild,'2026.08.14-pwa280');
  }
});
