...
test('SDK-generated assembly metadata replaces manual assembly attributes', () => {
  ...
  for (const project of [launcherProject, hostProject]) {
    ...
    assert.match(project, /<AssemblyVersion>1\.67\.1\.0<\/AssemblyVersion>/);
    assert.match(project, /<FileVersion>1\.67\.1\.0<\/FileVersion>/);
    assert.match(project, /<InformationalVersion>1\.67\.1-(?:launcher87|serverhost60)-csharp<\/InformationalVersion>/);
  }
