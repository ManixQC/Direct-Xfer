...
test('1.71.17 release and pwa480 are synchronized',()=>{
  assert.match(read('package.json'),/"version"\s*:\s*"1\.71\.17"/);
  for(const f of ['pwa/app.js','pwa/index.html','pwa/sw.js','pwa/theme-init.js','pwa/admin-advanced.js','pwa/mobile-intelligence.js']) assert.match(read(f),/1\.71\.17|pwa480|v=461/);
...
test('shortcut manifests use the current pwa480 cache buster on app and login paths',()=>{
  assert.match(app,/manifestHref = '\/direct-xfer-pwa\.webmanifest\?v=461'/);
  assert.match(read('pwa/login.html'),/direct-xfer-pwa\.webmanifest\?v=461/);
});
