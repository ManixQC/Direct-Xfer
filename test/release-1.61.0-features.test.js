...
test('1.63.4 share editing and duplication retain useful config while clearing runtime state', () => {
...
test('1.63.4 standard share search covers type, files, recipients and recent client metadata', () => {
...
test('1.63.4 detailed stats support selectable periods, comparison, failure causes and resume visibility', () => {
...
test('1.63.4 reception shows restrictions and aggregate speed/ETA progress before and during upload', () => {
...
test('1.63.4 reception resumes per file and detects duplicates with replace keep or ignore choices', () => {
...
test('1.63.4 image editor keeps versions, edit history, restore and before-after comparison', () => {
...
test('1.63.4 admin before-after preview does not use the public view-counting image route', () => {
  ...
  // route choice instead of depending on checkout EOL policy.
  const normalizedApp = app.replace(/\r\n/g, '\n').replace(/\r/g, '\n');
  const start = normalizedApp.indexOf('async function openPhotoVersions');
  const end = normalizedApp.indexOf('\n}\n', start) + 3;
  const fn = normalizedApp.slice(start, end);
  assert.match(fn, /\/api\/photos\/.*\/preview/);
  assert.doesNotMatch(fn, /cur\.src='\/i\//);
});

test('1.63.4 image variant stats include bandwidth and view share and edits bump cache revisions', () => {
