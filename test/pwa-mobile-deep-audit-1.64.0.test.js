...
test('1.70.27 release and pwa460 are synchronized',()=>{
  assert.match(read('package.json'),/"version"\s*:\s*"1\.70\.27"/);
  for(const f of ['pwa/app.js','pwa/index.html','pwa/sw.js','pwa/theme-init.js','pwa/admin-advanced.js','pwa/mobile-intelligence.js']) assert.match(read(f),/1\.70\.27|pwa460|v=443/);
...
test('shortcut manifests use the current pwa460 cache buster on app and login paths',()=>{
  assert.match(app,/manifestHref = '\/direct-xfer-pwa\.webmanifest\?v=443'/);
  assert.match(read('pwa/login.html'),/direct-xfer-pwa\.webmanifest\?v=443/);
});
