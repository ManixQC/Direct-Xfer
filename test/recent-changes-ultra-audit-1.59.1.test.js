...
test('1.59.1 release metadata is synchronized across Node, PWA, C# and installer', () => {
  ...
  assert.match(workflow, /DX_VERSION: '1\.59\.1'/);
  assert.match(workflow, /DX_RUNTIME_BUILD: '1\.59\.1-launcher27-csharp'/);
  assert.match(iss, /#define AppVersion "1\.59\.1"/);
});

test('PWA cache generation is advanced consistently for 1.59.1', () => {
  ...
  assert.match(read('pwa', 'index.html'), /v=266/);
  assert.match(read('pwa', 'app.js'), /v=266/);
  assert.match(read('pwa', 'sw.js'), /v=266/);
});
...
test('bundled hash-pinned Node.js is preferred and version probing has a hard timeout', () => {
  const candidates = launcher.slice(launcher.indexOf('private static IEnumerable<string> NodeCandidates()'), launcher.indexOf('private string EnsureNode()'));
  ...
  assert.match(launcher, /process\.ExitCode != 0/);
});
...
test('GitHub workflow has one release source of truth for artifact names and reproducible build Node', () => {
  ...
  assert.doesNotMatch(workflow, /name: Direct-Xfer-Setup-1\.59\.1/);
});

test('all launcher critical runtime hashes match the actual normalized 1.59.1 source files', () => {
  const files = [
    'package.json',
    'package-lock.json',
    'server.js',
...
test('GitHub Windows release build is gated by the latest regression suite', () => {
  ...
  assert.match(gate, /recent-changes-ultra-audit-1\.59\.1\.test\.js/);
});
