...
test('Windows metadata targets 1.66.1 and current runtime hashes', () => {
...
test('Windows GitHub Actions run name follows Direct-Xfer 1.66.1', () => {
  const workflow = read('.github/workflows/build-windows-csharp.yml');
  assert.match(workflow, /^run-name: v1\.66\.1$/m);
});
