...
test('device-only status lists all devices paired to its account', async()=>{
  ...
  for (const current of [phone, tablet]) {
    ...
    const statusUrl=`${base}/app/device/status?version=${encodeURIComponent('1.62.3')}&build=${encodeURIComponent('2026.08.16-pwa307')}`;
    ...
    assert.equal(currentDevice.appVersion,'1.62.3');
    assert.equal(currentDevice.appBuild,'2026.08.16-pwa307');
  }
});
