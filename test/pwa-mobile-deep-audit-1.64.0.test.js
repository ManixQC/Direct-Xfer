...
test('1.66.6 release and pwa356 are synchronized',()=>{
  assert.match(read('package.json'),/"version"\s*:\s*"1\.66\.6"/);
  for(const f of ['pwa/app.js','pwa/index.html','pwa/sw.js','pwa/theme-init.js','pwa/admin-advanced.js','pwa/mobile-intelligence.js']) assert.match(read(f),/1\.66\.6|pwa356|v=356/);
...
test('shortcut manifests use the current pwa356 cache buster on app and login paths',()=>{
  assert.match(app,/direct-xfer-pwa\.webmanifest'[\s\S]*?\+ '\?v=356'/);
  assert.match(read('pwa/login.html'),/direct-xfer-pwa\.webmanifest\?v=356/);
});
