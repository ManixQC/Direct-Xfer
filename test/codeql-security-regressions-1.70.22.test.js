...
test('PWA security hotfix advances the shell and login cache generations without changing app version', () => {
  ...
  assert.match(sw, /app\.js\?v=450/);
  assert.match(app, /APP_VERSION = '1\.71\.6'/);
  assert.match(app, /APP_BUILD = '2026\.08\.25-pwa469'/);
  assert.match(loginHtml, /login\.js\?v=321/);
  assert.match(loginHtml, /login-vault\.js\?v=450/);
