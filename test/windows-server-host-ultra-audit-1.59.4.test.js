...
test('1.62.2 Windows server metadata is synchronized', () => {
  assert.match(host, /AppVersion = "1\.62\.2"/);
  ...
  assert.match(launcher, /ServerHostBuild = "1\.62\.2-serverhost16-csharp"/);
...
test('all ServerHost critical runtime hashes match the exact normalized 1.62.2 runtime files', () => {
