...
test('Windows metadata targets 1.65.1 and current runtime hashes', () => {
  const launcher = read('windows-launcher/Program.cs');
  const host = read('windows-server-host/Program.cs');
  assert.match(launcher, /AppVersion = "1\.65\.1"/);
  assert.match(launcher, /RuntimeAppBuild = "1\.65\.1-launcher62-csharp"/);
  assert.match(host, /HostVersion = "1\.65\.1-serverhost35-csharp"/);
