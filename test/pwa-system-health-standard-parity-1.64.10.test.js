...
test('PWA System Health reuses the standard dashboard markup and renderer', () => {
  ...
  assert.match(pwa, /server-health-dashboard\.css\?v=444/);
  assert.match(pwa, /server-health-dashboard\.js\?v=444/);
  ...
  assert.match(sw, /server-health-dashboard\.css\?v=444/);
  assert.match(sw, /server-health-dashboard\.js\?v=444/);
});
