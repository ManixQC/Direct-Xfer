...
test('device-only status lists all devices paired to its account', async()=>{
  ...
  for (const current of [phone, tablet]) {
    ...
    const statusUrl=`${base}/app/device/status?version=${encodeURIComponent('1.59.5')}&build=${encodeURIComponent('2026.08.14-pwa284')}`;
    ...
    assert.equal(currentDevice.appVersion,'1.59.5');
    assert.equal(currentDevice.appBuild,'2026.08.14-pwa284');
  }
});
