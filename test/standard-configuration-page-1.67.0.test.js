...
test('1.67.1 Configuration is a dedicated full page instead of an overlay modal', () => {
...
test('1.67.1 Configuration has a real URL and participates in SPA history navigation', () => {
...
test('1.67.1 Configuration page is owner/admin only and moves shared account controls into its top bar', () => {
...
test('1.67.1 optional rclone absence cannot turn connector listing into an HTTP 500', () => {
...
test('1.67.1 connector job polling follows Configuration page visibility, not the removed modal', () => {
  assert.match(app, /\['queued','running'\]\.includes\(job\.status\)\) && configPageOpen\(\)/);
  assert.doesNotMatch(app, /config-overlay/);
});

test('1.67.1 logout clears sensitive Configuration-page fields and account-specific connector output', () => {
