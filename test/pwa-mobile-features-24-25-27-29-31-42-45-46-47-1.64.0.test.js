...
test('pwa488 loads and precaches the mobile intelligence module as a public shell asset', () => {
  ...
  assert.match(index, /mobile-intelligence\.js\?v=469/);
  assert.match(sw, /\/app\/mobile-intelligence\.js\?v=469/);
...
test('fresh unauthenticated PWA bootstrap can fetch the new module and manifests', { timeout:25000 }, async () => {
  ...
  try {
    ...
    const mod = await fetch(base + '/app/mobile-intelligence.js?v=469');
    ...
    for (const name of ['manifest.webmanifest','manifest-en.webmanifest','manifest-es.webmanifest']) {
      const r = await fetch(base + '/app/' + name + '?v=469');
